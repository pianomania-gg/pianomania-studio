...
echo "Prune nonessential Qt SQL drivers"
rm -f \
    "${VOLUME}/${APPNAME}.app/Contents/PlugIns/sqldrivers/libqsqlmimer.dylib" \
    "${VOLUME}/${APPNAME}.app/Contents/PlugIns/sqldrivers/libqsqlodbc.dylib" \
    "${VOLUME}/${APPNAME}.app/Contents/PlugIns/sqldrivers/libqsqlpsql.dylib"

echo "Bundle external Homebrew dylib dependencies"
bash buildscripts/packaging/macOS/bundle-homebrew-dylibs.sh "${VOLUME}/${APPNAME}.app"
...
if $DO_SIGN; then
    ...
    # Sign bundled Homebrew dylibs before the app bundle seal is created.
    echo "Sign bundled Homebrew dylibs"
    find "${APP_PATH}/Contents/Frameworks" -maxdepth 1 -type f \( -name '*.dylib' -o -name '*.so' \) -exec codesign --force --options runtime -s "Developer ID Application: MuseScore" '{}' ';'
