...
for required_tool in chmod cp file find install_name_tool lipo mkdir otool sort; do
...
dependency_id() {
    local file_path="$1"
    otool -D "$file_path" 2>/dev/null | sed -E '/:$/d' | sed -n '1p'
}

dependencies() {
    local file_path="$1"
    otool -L "$file_path" | sed -E '/:$/d' | sed -E 's/^[[:space:]]*//' | sed -E 's/[[:space:]]+\(.*$//'
}
