...
usage() {
    cat <<'EOF'
Usage:
  package-macos-dev.sh --build-dir <path> --qt-dir <path> --output-app <path> --output-zip <path> [--sign-identity <identity>] [--notarize ...]

Options:
  --build-dir      Build directory containing install/mscore.app.
  --qt-dir         Qt installation root, for example ~/Qt/6.9.3/macos.
...
for required_tool in codesign ditto find otool; do
