        ...
        if (sel) {
            ...
            if (!notes.empty() && notes[0]) {
                ...
                if (undoStack) {
                    ...
                    for (auto& note : notes) {
                        if (note) {
                            // Toggle the property - if it's currently true, set it to false, and vice versa
                            bool currentValue = note->getProperty(mu::engraving::Pid::PIANOMANIA_HELD_NOTE).toBool();
                            bool newValue = !currentValue;
                            note->undoChangeProperty(mu::engraving::Pid::PIANOMANIA_HELD_NOTE, newValue, mu::engraving::PropertyFlags::UNSTYLED);
                        }
                    }
