...
bool NotationInteraction::applyPaletteElement(mu::engraving::EngravingItem* element, Qt::KeyboardModifiers modifiers)
{
    ...
    if (sel.isList()) {
        ...
        auto chordForElement = [](EngravingItem* e) -> Chord* {
            if (e->isNote()) {
        ...
        Chord* turnStart = (sel.elements().size() == 2) ? chordForElement(sel.elements()[0]) : nullptr;
        Chord* turnEnd = (sel.elements().size() == 2) ? chordForElement(sel.elements()[1]) : nullptr;
        bool isTurnBetweenNotes = element->isOrnament()
                                  && toArticulation(element)->symId() == SymId::ornamentTurn
                                  && turnStart && turnEnd
                                  && turnStart != turnEnd
                                  && turnStart->staffIdx() == turnEnd->staffIdx()
                                  && turnStart->tick() != turnEnd->tick();
        ...
        } else if (isTurnBetweenNotes) {
            score->toggleOrnament(SymId::ornamentTurn);
