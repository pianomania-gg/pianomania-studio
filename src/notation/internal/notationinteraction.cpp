...
void NotationInteraction::addRubatoZoneToSelection()
{
    if (selection()->isNone()) {
        return;
    }

    startEdit(TranslatableString("undoableAction", "Add rubato zone"));
    if (!score()->cmdAddRubatoZone()) {
        rollback();
        interactive()->error(
            muse::trc("notation", "Cannot add rubato zone"),
            muse::trc("notation", "Select one or more complete measures to add a rubato zone."));
        return;
    }
    apply();
}

void NotationInteraction::addPyroSpanToSelection()
{
    if (selection()->isNone()) {
        return;
    }

    startEdit(TranslatableString("undoableAction", "Add pyro span"));
    if (!score()->cmdAddPyroSpan()) {
        rollback();
        interactive()->error(
            muse::trc("notation", "Cannot add pyro span"),
            muse::trc("notation", "Select a range with a positive duration. Pyro spans cannot overlap each other."));
        return;
    }
    apply();
}

void NotationInteraction::addLaserSpanToSelection()
{
    if (selection()->isNone()) {
        return;
    }

    startEdit(TranslatableString("undoableAction", "Add laser span"));
    if (!score()->cmdAddLaserSpan()) {
        rollback();
        interactive()->error(
            muse::trc("notation", "Cannot add laser span"),
            muse::trc("notation", "Select a note range starting and ending on notes; laser spans may not overlap each other."));
        return;
    }
    apply();
}
...
void NotationInteraction::applyPianomaniaAutoLayout()
{
    TRACEFUNC;

    startEdit(TranslatableString("undoableAction", "Apply Pianomania Auto Layout"));

    // Plan breaks against prettified geometry: the slur/fingering prettify
    // passes grow system skylines, and a page plan computed on vanilla
    // geometry can overflow (and restructure) once Prettify persists those
    // offsets. Keeping the transient prettify passes on while planning makes
    // the plan valid for the score's final look, so Prettify stays
    // layout-stable.
    ...
    mu::engraving::pm::applyPianomaniaAutoLayout(score()->masterScore());
    apply();
}

void NotationInteraction::applyPianomaniaPrettify()
{
    TRACEFUNC;

    startEdit(TranslatableString("undoableAction", "Prettify Pianomania slurs and fingerings"));

    const mu::engraving::pm::PmPrettifyResult result = mu::engraving::pm::applyPianomaniaPrettify(score());

    LOGI() << "Pianomania prettify: manual slurs normalized="
           ...
           << result.manualReviewFingerings;

    if (result.structuralAssignmentChanged) {
        LOGW() << "Pianomania prettify rolled back because it changed the structural layout";
        rollback();
        return;
    }

    if (!result.changed) {
        LOGI() << "Pianomania prettify: no persisted changes";
        rollback();
    } else {
        apply();
    }
