...
void GlobalModule::onPreInit(const IApplication::RunMode& mode)
{
    ...
    //! Console
#ifdef Q_OS_WASM
    // Headless wasm converter: no on-disk logging (MEMFS is throwaway). Log to
    // console (stderr) so diagnostics are still visible to the host.
...
#if !defined(MUSE_CONFIGURATION_IS_WEB) && !defined(Q_OS_WASM)
