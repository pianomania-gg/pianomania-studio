...
void GlobalModule::onPreInit(const IApplication::RunMode& mode)
{
    ...
    io::path_t logFilePath;
#ifdef Q_OS_WASM
    // Headless wasm converter: no on-disk logging (MEMFS is throwaway, and the log
    // rotation / file-path construction traps here). Log to console (stderr) so
    // diagnostics are still visible to the host.
    logger->addDest(new ConsoleLogDest(LogLayout("${time} | ${type|5} | ${tag|15} | ${message}")));
#else
    logFilePath = logPath;
