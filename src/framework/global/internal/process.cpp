...
#ifndef Q_OS_WASM
#include <QProcess> // not available in Qt for WebAssembly (no subprocesses in a browser)
#endif
...
int Process::execute(const std::string& program, const std::vector<std::string>& args)
{
    ...
    return ret;
#else
    // No subprocess support in the browser; the headless converter never spawns one.
    Q_UNUSED(program);
    Q_UNUSED(args);
    return -1;
#endif
