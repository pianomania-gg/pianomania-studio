...
#ifdef Q_OS_WASM
    // Headless converter: use the FreeType font provider directly. The default
    // dispatcher routes addSymbolFont + text metrics to QFontProvider
    // (QFontDatabase), which needs a QGuiApplication; we run QCoreApplication. The
    // FT provider implements every IFontProvider method via the fonts engine
    // (metrics only — msdfgen rendering isn't needed for MEI/MIDI export).
    ioc()->registerExport<draw::IFontProvider>(moduleName(), mainFProvider);
#else
