...
void DrawModule::registerExports()
{
    ...
    // QFontProvider routes addSymbolFont + text metrics through QFontDatabase,
    // which needs a QGuiApplication; we run QCoreApplication. The FT provider
    // implements every IFontProvider method via the fonts engine (metrics only —
    // msdfgen rendering isn't needed for MEI/MIDI export).
