...
void AudioEngine::deinit()
{
    ...
    //! NOTE Release unconditionally. The mixer owns async channels bound to this
    //! thread; if init had not completed when deinit arrives (console conversion,
    //! where init and deinit race), guarding on m_inited left the mixer alive to be
    //! destroyed on the main thread at process exit, aborting after a successful run.
    m_mixer = nullptr;
    LOGD() << "deinited";
}
