...
{
#ifndef Q_OS_WASM
    //! A fast console conversion can finish while EngineRunning is still queued
    //! for the main thread. Disconnect its deferred EngineInit callback before
    //! queuing EngineDeinit. An EngineInit already sent to the worker remains
    //! ordered before EngineDeinit and completes normal teardown.
    m_isEngineRunning.ch.disconnect(this);

    auto engineDeinited = std::make_shared<bool>(false);
    m_rpcChannel->send(rpc::make_request(Method::EngineDeinit), [this, engineDeinited](const Msg&) {
        *engineDeinited = true;
    ...
    //! NOTE Wait for the engine to actually deinit, not merely for playback to stop.
    //! The engine clears its async channels on the audio thread in EngineController::deinit().
    //! When audio never started (console conversion), m_isAudioStarted is already false, so
    //! waiting on it alone let us stop the worker before that ran; the channels were then
    //! destroyed on the main thread against a dead thread queue, aborting the process after
    ...
    do {
        ...
        if (*engineDeinited && !m_isAudioStarted.val) {
            break;
        }
    ...
    if (!*engineDeinited) {
        LOGE() << "audio engine did not confirm deinit; stopping worker anyway";
    }
