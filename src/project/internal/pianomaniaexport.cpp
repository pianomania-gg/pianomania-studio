...
std::string midiKindName(MidiExportedFile::Kind kind)
{
    ...
    case MidiExportedFile::Kind::Base:
        return "base";
    case MidiExportedFile::Kind::NoRepeats:
        return "no-repeats";
                          ...
                          std::vector<CanonicalEvent>& destination)
{
    ...
    for (const mu::iex::mei::PmMeiNoteRecord& record : records) {
        ...
        if (primary == primaryByLocator.end()) {
            ...
            const CanonicalEvent& target = destination[primary->second];
                        ...
                        std::vector<BuiltFile>& builtFiles)
{
    ...
    if (!baseFile) {
        LOGE() << "Pianomania manifest v2 requires a base MIDI file";
        return false;
    }
    ...
    for (const MidiExportedFile::AudibleEvent& event : baseFile->audibleEvents) {
        ...
        if (event.origins.size() != 1 || !event.origins.front().nonVisualOrnament) {
            LOGE() << "Each score-declared ornament attack must have one stable target origin";
            return false;
        }
    ...
    for (const MidiExportedFile& file : result.files) {
        ...
        for (const BuiltAudibleEvent& audible : built.audibleEvents) {
            if (audible.eventType.empty() || audible.occurrenceIds.empty()) {
                LOGE() << "Exported audible event is not fully classified in manifest v2";
                return false;
            }
        }
...
bool buildManifestJson(PianomaniaExportResult& result)
{
    ...
    QJsonObject root;
    root["schema_version"] = "a2p.exporter-provenance.v2";
    root["score"] = score;
    ...
    bool exportRpns)
{
    ...
    if (!mu::iex::mei::pmWriteMeiToString(score, true, meiData, &canonicalRecords)
        || !buildCanonicalEvents(canonicalRecords, rv.val.canonicalEvents)) {
        rv.ret = muse::make_ret(muse::Ret::Code::UnknownError,
    ...
    if (rv.val.repeatInfo.hasMultipleEndings && rv.val.repeatInfo.finalEndingNumber > 0) {
        ...
        if (!mu::iex::mei::pmWriteMeiToString(noRepeatScore.get(), false, ignoredMei, &noRepeatRecords)) {
            rv.ret = muse::make_ret(muse::Ret::Code::UnknownError,
    ...
    const auto writeVariant = [&](mu::engraving::Score* exportScore,
                                  ...
                                  const std::vector<mu::iex::mei::PmMeiNoteRecord>& records) -> bool {
        ...
        file.variantCanonicalEvents = variantCanonicalEvents(records);
    ...
    if (!writeVariant(score, basePath + ".mid", false, MidiExportedFile::Kind::Base, canonicalRecords)) {
        rv.ret = muse::make_ret(muse::Ret::Code::UnknownError,
                                std::string("Failed to export base Pianomania variant"));
        return rv;
    }
    if (noRepeatScore
        && !writeVariant(noRepeatScore.get(), basePath + "-no-repeats.mid", false,
                         MidiExportedFile::Kind::NoRepeats, noRepeatRecords)) {
        rv.ret = muse::make_ret(muse::Ret::Code::UnknownError,
                                std::string("Failed to export no-repeat Pianomania variant"));
        return rv;
    }
    ...
    if (!buildManifestJson(rv.val)) {
        rv.ret = muse::make_ret(muse::Ret::Code::UnknownError,
                                std::string("Failed to assemble Pianomania export manifest v2"));
        return rv;
    }
