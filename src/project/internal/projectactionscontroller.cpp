...
namespace {
bool writeMeiFile(mu::engraving::Score* score, const muse::io::path_t& path)
{
    if (!score) {
        return false;
    }

    mu::iex::mei::MeiWriter writer;
    mu::engraving::Err err = writer.writeScore(score, path);
    if (err != mu::engraving::Err::NoError) {
        LOGE() << "Failed to export MEI: " << path.toQString();
        return false;
    }
                                                      ...
                                                      bool exportPdf, bool exportMidi, bool exportMei)
{
    ...
    if (exportMidi) {
        pianomania::writeMidiFile(score, basePath + ".mid", false, exportRpns);

        if (noRepeatScore) {
            pianomania::writeMidiFile(noRepeatScore.get(), basePath + "-no-repeats.mid", false, exportRpns);
        }
    ...
    if (exportMei) {
        ...
        if (noRepeatScore) {
            writeMeiFile(noRepeatScore.get(), basePath + "-no-repeats.mei");
        }
    }
}
...
void ProjectActionsController::exportPianomania()
{
    ...
    muse::io::path_t dir = interactive()->selectDirectory(muse::qtrc("project/export", "Select export folder"), "");
    if (dir.empty()) {
        return;
    }
...
void ProjectActionsController::exportPianomaniaAll()
{
    ...
    muse::io::path_t root = interactive()->selectDirectory(muse::qtrc("project/export", "Select root folder"), "");
    if (root.empty()) {
        return;
    }
