...
namespace compatibility = mu::project::pianomania::compatibility;

static QString pianomaniaOptionName(const char* commandOption)
{
    return QString::fromUtf8(commandOption + 2);
}
...
void CommandLineParser::init()
{
    ...
    m_parser.addOption(QCommandLineOption(pianomaniaOptionName(compatibility::EXPORT_MEI_COMMAND_OPTION),
                                          "Write the coordinated Pianomania MEI export to this file", "meiPath"));
    m_parser.addOption(QCommandLineOption(pianomaniaOptionName(compatibility::EXPORTER_MANIFEST_V3_COMMAND_OPTION),
                                          "Write the coordinated Pianomania export manifest to this JSON file", "manifestPath"));
    m_parser.addOption(QCommandLineOption(pianomaniaOptionName(compatibility::EXPORT_PDF_COMMAND_OPTION),
                                          "Write the coordinated Pianomania engraved PDF export to this file", "pdfPath"));
    m_parser.addOption(QCommandLineOption(pianomaniaOptionName(compatibility::FORCE_NORMALIZE_SLURS_FINGERINGS_COMMAND_OPTION),
                                          "Force Pianomania slur/fingering auto-prettify to replace manual slur grips and fingering offsets"));
    m_parser.addOption(QCommandLineOption(pianomaniaOptionName(compatibility::AUTO_LAYOUT_COMMAND_OPTION),
...
//    m_parser.addOption(QCommandLineOption("piano-position", "Show Piano top or bottom. Default bottom", "bottom"));
    ...
    m_parser.addOption(internalCommandLineOption("pianomania-release-smoke-test",
                                                 "Launch the complete UI, then exit cleanly for release verification"));
}
...
void CommandLineParser::parse(int argc, char** argv)
{
    ...
    m_options.app.pianomaniaReleaseSmokeTest = m_parser.isSet("pianomania-release-smoke-test");
    ...
    if (m_parser.isSet(pianomaniaOptionName(compatibility::EXPORT_MIDI_COMMAND_OPTION))
        ...
        || m_parser.isSet(pianomaniaOptionName(compatibility::EXPORT_PDF_COMMAND_OPTION))) {
        m_options.runMode = IApplication::RunMode::ConsoleApp;
        m_options.converterTask.type = ConvertType::ExportPianomaniaMidi;

        if (scorefiles.size() != 1) {
            LOGE() << "Option: " << compatibility::EXPORT_MIDI_COMMAND_OPTION
                   << " expects exactly one input .mscz score";
        } else if (io::suffix(scorefiles[0].toStdString()) != mu::engraving::MSCZ) {
            LOGE() << "Option: " << compatibility::EXPORT_MIDI_COMMAND_OPTION
                   << " input score must be .mscz";
        } else {
            m_options.converterTask.inputFile = scorefiles[0];
        }
        ...
        if (!m_parser.isSet(pianomaniaOptionName(compatibility::EXPORT_MIDI_COMMAND_OPTION))) {
            LOGE() << "Option: " << compatibility::EXPORT_MIDI_COMMAND_OPTION
                   << " is required for Pianomania export";
        } else {
            m_options.converterTask.outputFile = fromUserInputPath(
                m_parser.value(pianomaniaOptionName(compatibility::EXPORT_MIDI_COMMAND_OPTION)));
        }

        if (!m_parser.isSet(pianomaniaOptionName(compatibility::EXPORT_MEI_COMMAND_OPTION))) {
            LOGE() << "Option: " << compatibility::EXPORT_MEI_COMMAND_OPTION
                   << " is required for Pianomania export";
        } else {
            m_options.converterTask.params[CmdOptions::ParamKey::PianomaniaExportMeiPath]
                = fromUserInputPath(m_parser.value(pianomaniaOptionName(compatibility::EXPORT_MEI_COMMAND_OPTION)));
        }

        if (!m_parser.isSet(pianomaniaOptionName(compatibility::EXPORTER_MANIFEST_V3_COMMAND_OPTION))) {
            LOGE() << "Option: " << compatibility::EXPORTER_MANIFEST_V3_COMMAND_OPTION
                   << " is required for Pianomania export";
        } else {
            m_options.converterTask.params[CmdOptions::ParamKey::PianomaniaExportManifestPath]
                = fromUserInputPath(m_parser.value(pianomaniaOptionName(compatibility::EXPORTER_MANIFEST_V3_COMMAND_OPTION)));
        }

        if (m_parser.isSet(pianomaniaOptionName(compatibility::EXPORT_PDF_COMMAND_OPTION))) {
            m_options.converterTask.params[CmdOptions::ParamKey::PianomaniaExportPdfPath]
                = fromUserInputPath(m_parser.value(pianomaniaOptionName(compatibility::EXPORT_PDF_COMMAND_OPTION)));
        }
    }

    if (m_parser.isSet(pianomaniaOptionName(compatibility::FORCE_NORMALIZE_SLURS_FINGERINGS_COMMAND_OPTION))) {
        m_options.converterTask.params[CmdOptions::ParamKey::PianomaniaForceNormalizeSlursFingerings] = true;
    }

    if (m_parser.isSet(pianomaniaOptionName(compatibility::AUTO_LAYOUT_COMMAND_OPTION))) {
        m_options.converterTask.params[CmdOptions::ParamKey::PianomaniaAutoLayout] = true;
    }
