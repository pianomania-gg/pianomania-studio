...
void CommandLineParser::parse(int argc, char** argv)
{
    ...
    if (m_parser.isSet("pianomania-export-midi-base")
        || m_parser.isSet("pianomania-export-mei")
        || m_parser.isSet("pianomania-export-manifest")) {
        m_options.runMode = IApplication::RunMode::ConsoleApp;
        m_options.converterTask.type = ConvertType::ExportPianomaniaMidi;

        if (scorefiles.size() != 1) {
            LOGE() << "Option: --pianomania-export-midi-base expects exactly one input .mscz score";
        } else if (io::suffix(scorefiles[0].toStdString()) != mu::engraving::MSCZ) {
            LOGE() << "Option: --pianomania-export-midi-base input score must be .mscz";
        } else {
            m_options.converterTask.inputFile = scorefiles[0];
        }

        if (!m_parser.isSet("pianomania-export-midi-base")) {
            LOGE() << "Option: --pianomania-export-midi-base is required for Pianomania export";
        } else {
            m_options.converterTask.outputFile = fromUserInputPath(m_parser.value("pianomania-export-midi-base"));
        }

        if (!m_parser.isSet("pianomania-export-mei")) {
            LOGE() << "Option: --pianomania-export-mei is required for Pianomania export";
        } else {
            m_options.converterTask.params[CmdOptions::ParamKey::PianomaniaExportMeiPath]
                = fromUserInputPath(m_parser.value("pianomania-export-mei"));
        }

        if (!m_parser.isSet("pianomania-export-manifest")) {
            LOGE() << "Option: --pianomania-export-manifest is required for Pianomania export";
        } else {
            m_options.converterTask.params[CmdOptions::ParamKey::PianomaniaExportManifestPath]
                = fromUserInputPath(m_parser.value("pianomania-export-manifest"));
        }
    }

    if (m_parser.isSet("pianomania-force-normalize-slurs-fingerings")) {
        m_options.converterTask.params[CmdOptions::ParamKey::PianomaniaForceNormalizeSlursFingerings] = true;
    }

    if (m_parser.isSet("pianomania-auto-layout")) {
        m_options.converterTask.params[CmdOptions::ParamKey::PianomaniaAutoLayout] = true;
    }

    if (m_parser.isSet("pianomania-prettify")) {
