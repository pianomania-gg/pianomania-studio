...
#else
    // Headless wasm converter: applyCommandLineOptions injects configs from editor
    // modules that aren't built here (ui/notation/project/audioexport/guitarpro/
    // musicxml), which would null-deref. We have no CLI options to apply anyway.
    (void)options;
#endif
...
#ifdef Q_OS_WASM
    // Headless converter: main() never enters the Qt event loop, but Qt's wasm
    // event dispatcher drains posted events from the JS event loop later
    // (QEventDispatcherWasm::wakeUp -> emscripten_async_call), so anything queued
    // here fires *after* main() returns. The CLI task dispatch below would then
    // call converter() — a null inject, since the converter module isn't built
    // for wasm — and trap as an uncaught "null function" RuntimeError in the
    // browser console at page load. Run startup synchronously and queue nothing;
    // the Embind pmConvert() is the only conversion entry point.
...
int ConsoleApp::processConverter(const CmdOptions::ConverterTask& task)
{
    ...
    bool pianomaniaAutoLayout = task.params[CmdOptions::ParamKey::PianomaniaAutoLayout].toBool();
    bool forceNormalizeSlursFingerings = task.params[CmdOptions::ParamKey::PianomaniaForceNormalizeSlursFingerings].toBool();
    bool pianomaniaPrettify = task.params[CmdOptions::ParamKey::PianomaniaPrettify].toBool();

    mu::engraving::MScore::pianomaniaPrettifySlursFingerings = forceNormalizeSlursFingerings;
    mu::engraving::MScore::pianomaniaForceNormalizeSlursFingerings = forceNormalizeSlursFingerings;
    mu::engraving::MScore::resetPianomaniaSlurFingeringDiagnostics();
    ...
    case ConvertType::File: {
        ...
        ret = converter()->fileConvert(task.inputFile, task.outputFile, openParams, soundProfile, tracksDiffPath,
                                       ...
                                       pianomaniaAutoLayout, pianomaniaPrettify);
    } break;
    ...
    case ConvertType::ExportPianomaniaMidi: {
        muse::io::path_t meiPath = task.params[CmdOptions::ParamKey::PianomaniaExportMeiPath].toString();
        muse::io::path_t manifestPath = task.params[CmdOptions::ParamKey::PianomaniaExportManifestPath].toString();
        muse::io::path_t pdfPath = task.params[CmdOptions::ParamKey::PianomaniaExportPdfPath].toString();
        ret = converter()->exportPianomaniaMidi(task.inputFile, task.outputFile, meiPath, manifestPath, pdfPath,
                                                openParams, pianomaniaAutoLayout, pianomaniaPrettify);
    } break;
    ...
    if (forceNormalizeSlursFingerings) {
        LOGI() << "Pianomania slur/fingering force-normalize: manual slurs normalized="
               << mu::engraving::MScore::pianomaniaNormalizedManualSlurs
               << ", manual fingerings normalized="
               << mu::engraving::MScore::pianomaniaNormalizedManualFingerings
               << ", fingering manual-review diagnostics="
               << mu::engraving::MScore::pianomaniaManualReviewFingerings;
    }
    mu::engraving::MScore::pianomaniaPrettifySlursFingerings = false;
    mu::engraving::MScore::pianomaniaForceNormalizeSlursFingerings = false;
