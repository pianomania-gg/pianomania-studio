...
#include <QtGlobal> // defines Q_OS_WASM (must precede the guard below)
#ifndef Q_OS_WASM
#include <QTextCodec> // Core5Compat — not available in Qt for WebAssembly
#endif
...
#define MUSE_APP_INSTALL_SUFFIX ""
    ...
    auto wasmOptions = commandLineParser.options();
    wasmOptions.runMode = IApplication::RunMode::ConsoleApp;
    std::shared_ptr<muse::IApplication> app = f.newApp(wasmOptions);
