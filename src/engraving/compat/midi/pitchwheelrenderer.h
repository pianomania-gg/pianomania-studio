...
public:
    ...
    void addPianomaniaPitchWheelFunction(const PitchWheelFunction& function, uint32_t channel, staff_idx_t staffIdx,
                                         MidiInstrumentEffect effect, std::string identity, int sensitivitySemitones);
...
private:
    ...
    struct PianomaniaPitchWheelRange {
        int32_t startTick = 0;
        int32_t endTick = 0;
        std::string identity;
        int sensitivitySemitones = 12;
    };
    std::unordered_map<uint32_t, std::vector<PianomaniaPitchWheelRange> > _pianomaniaRanges;
    std::unordered_map<uint32_t, std::vector<std::pair<int32_t, int32_t> > > _otherRanges;
};
}
