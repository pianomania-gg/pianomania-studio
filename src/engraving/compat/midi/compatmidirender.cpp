...
{
    found = false;
    if (!note || !ornament || diatonicSteps == 0) {
        return 0;
    }

    if (diatonicSteps == 1 && ornament->hasIntervalAbove()) {
        if (const Note* upperNote = ornament->noteAbove()) {
            found = true;
            return upperNote->pitch() - note->pitch();
        }

        OrnamentInterval interval = ornament->intervalAbove();
    ...
    if (diatonicSteps == -1 && ornament->hasIntervalBelow()) {
        if (const Note* lowerNote = ornament->noteBelow()) {
            found = true;
            return lowerNote->pitch() - note->pitch();
        }

        OrnamentInterval interval = ornament->intervalBelow();
        if (interval.type != IntervalType::AUTO) {
            found = true;
            return -Interval::fromOrnamentInterval(interval).chromatic;
        }
...
static int ornamentPitchOffset(const Note* note, const Ornament* ornament, int diatonicSteps)
{
    bool foundExplicitPitch = false;
    int pitchOffset = explicitOrnamentPitchOffset(note, ornament, diatonicSteps, foundExplicitPitch);
    if (foundExplicitPitch) {
        return pitchOffset;
    }

    return chromaticPitchSteps(note, note, diatonicSteps);
}
                                                         ...
                                                         int gateTime, int ontime, int trailtime)
{
    ...
    for (size_t i : getNotesIndexesToRender(chord)) {
        ...
        const int noteGateTime = note->pianomaniaHeldNote() ? 100 : gateTime;
        for (NoteEvent& e : *el) {
            e.setLen(e.len() * noteGateTime / 100);
        }
                                              ...
                                              const Ornament* ornament /* = nullptr */)
{
    ...
    bool glissandoStart = noteIsGlissandoStart(note);
    int gnb = glissandoStart ? 0 : int(note->chord()->graceNotesBefore().size());
    ...
    if (!CompatMidiRendererInternal::graceNotesMerged(note->chord())) {
        gnb = 0;
        gna = 0;
    }
    ...
    if (graceOnBeatProportion > 0.0 && space > 0 && !glissandoStart) {
        int graceOffset = std::min(space, static_cast<int>(std::lround(graceOnBeatProportion * space)));
        if (graceOffset >= space) {
            return false;
        }
        space -= graceOffset;
        ontime = graceOffset;
    ...
    auto makeEvent
        = [note, chord, chromatic, events, ornament](int pitch, int ontime, int duration,
        ...
        events->push_back(NoteEvent(chromatic ? pitch : ornamentPitchOffset(note, ornament, pitch),
...
bool CompatMidiRender::renderNoteArticulation(NoteEventList* events, Note* note, bool chromatic, SymId articulationType,
                                              OrnamentStyle ornamentStyle, double graceOnBeatProportion, const Ornament* ornament)
{
    ...
    for (auto& oe : excursions) {
        if (oe.atype == articulationType && (0 == oe.ostyles.size()
            ...
            return renderNoteArticulation(events, note, chromatic, oe.duration,
                                          ...
                                          64, 8, graceOnBeatProportion, false, ornament);
        }
    }
    return false;
}
                                               ...
                                               double graceOnBeatProportion)
{
    ...
    if (trill->trillType() != TrillType::TRILL_LINE && trill->trillType() != TrillType::PRALLPRALL_LINE) {
        return CompatMidiRender::renderNoteArticulation(events, note, false, trill->trillType(), trill->ornamentStyle(),
                                               ...
                                               const Ornament* ornament, double graceOnBeatProportion)
{
    ...
    const int spanTicks = CompatMidiRender::totalTiedNoteTicks(note);
    if (chordTicks <= 0 || spanTicks <= 1) {
        return false;
    }

    int startTick = 0;
    if (graceOnBeatProportion > 0.0) {
    ...
    int upperPitchOffset = ornamentPitchOffset(note, ornament, 1);
    if (upperPitchOffset == 0) {
        upperPitchOffset = chromaticPitchSteps(note, note, 1);
    }
    if (upperPitchOffset == 0) {
        return false;
    }
    ...
    auto toPermille = [chordTicks](int ticks) {
        return static_cast<int>((static_cast<long long>(ticks) * NoteEvent::NOTE_LENGTH) / chordTicks);
    };
    ...
    events->clear();
    events->insert(events->end(), trillEvents.begin(), trillEvents.end());
    return true;
}
                                              ...
                                              OrnamentStyle ornamentStyle, double graceOnBeatProportion, const Ornament* ornament)
{
    std::map<TrillType, SymId> articulationMap = {
        ...
        { TrillType::TURN_BETWEEN,    SymId::ornamentTurn }
    };
    auto it = articulationMap.find(trillType);
    if (it == articulationMap.cend()) {
        return false;
    } else {
        return CompatMidiRender::renderNoteArticulation(events, note, chromatic, it->second, ornamentStyle, graceOnBeatProportion,
                                                        ornament);
    }
}
