    ...
    for (size_t i = 0; i < size(); ++i) {
        for (const auto& eventPair : _channels[i]) {
            ...
            if (event.type() == ME_NOTEON && event.velo() != 0) {
                const auto eventsAtNoteOn = pitchWheelEvents[i].equal_range(tick);
                const bool hasCenterAtNoteOn = std::any_of(eventsAtNoteOn.first, eventsAtNoteOn.second, [](const auto& candidate) {
                    return candidate.second.type() == ME_PITCHBEND
                           && candidate.second.dataA() == 0
                           && candidate.second.dataB() == 64;
                });
                if (hasCenterAtNoteOn) {
                    continue;
                }
...
void EventsHolder::fixupMIDI()
{
    ...
    for (auto& mm : _channels) {
        ...
        while (it != mm.end()) {
            ...
            bool isNoteOn = event.type() == ME_NOTEON && event.velo() > 0;
            bool isNoteOff = event.type() == ME_NOTEOFF || (event.type() == ME_NOTEON && event.velo() == 0);
