                        ...
                        PitchWheelRenderer& pitchWheelRenderer, const CompatMidiRendererInternal::Context& context)
{
    ...
    bool applyOrnamentNoteOff = hasOrnament && !hasArpeggio && !isGrace;
    ...
    if (applyOrnamentNoteOff && nels > 1) {
        int bestAbsPitch = std::numeric_limits<int>::max();
        int bestLen = std::numeric_limits<int>::min();
        ...
        for (size_t i = 0; i < nels; ++i) {
            ...
            int absPitch = std::abs(nel[i].pitch());
            int lenValue = nel[i].len();
            int ontimeValue = nel[i].ontime();
            if (absPitch < bestAbsPitch
                || (absPitch == bestAbsPitch && lenValue > bestLen)
                || (absPitch == bestAbsPitch && lenValue == bestLen && ontimeValue < bestOntime)) {
                bestAbsPitch = absPitch;
                bestLen = lenValue;
                bestOntime = ontimeValue;
                mainEventIndex = static_cast<int>(i);
            }
        }
    }
    for (size_t i = 0; i < nels; ++i) {
        ...
        if (e.play()) {
            ...
            // Pianomania reserves note-off velocity 127 for generated ornament playback
            ...
            } else if (applyOrnamentNoteOff && mainEventIndex >= 0 && static_cast<int>(i) != mainEventIndex) {
                playParams.noteOffVelocity = ORNAMENT_NOTE_OFF_VELOCITY;
            }
                          ...
                          const CompatMidiRendererInternal::Context& context)
{
    ...
    for (int p : pitches) {
        ...
        NPlayEvent offEv(ME_NOTEOFF, static_cast<uint8_t>(channel), p, DEFAULT_NOTE_OFF_VELOCITY);
        offEv.setHarmony(h);
                                    ...
                                    MidiInstrumentEffect effect, const CompatMidiRendererInternal::Context& context)
{
    ...
    for (const NoteEvent& e : turnEvents) {
        ...
        int velo = context.velocitiesByStaff.at(staff->idx()).val(nonUnwoundTick) * veloMultiplier * e.velocityMultiplier();
