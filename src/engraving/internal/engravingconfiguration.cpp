...
{
    static const SizeF size(10.0, 7.5);
    return size;
}

muse::String EngravingConfiguration::iconsFontFamily() const
{
    // uiConfiguration is null in headless builds (UI module not present, e.g. the
    // wasm converter). Layout still queries the icons font for break symbols, so
    // return the default family name.
    return uiConfiguration() ? String::fromStdString(uiConfiguration()->iconsFontFamily())
           : String(u"MusescoreIcon");
}
...
{
    if (!uiConfiguration()) {
        return Color::BLACK; // headless (no UI module): no theme available
    }
