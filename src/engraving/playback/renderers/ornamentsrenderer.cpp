...
// Keep audible trill playback aligned with the bounded MIDI export disclosure.
...
static bool isBoundedTrillType(const ArticulationType type)
{
    return type == ArticulationType::Trill || type == ArticulationType::TrillBaroque;
}

static int boundedTrillTicksPerNote(const double beatsPerSecond)
{
    const int ticksPerSecond = std::max(1, static_cast<int>(std::lround(beatsPerSecond * Constants::DIVISION)));
...
static int boundedTrillSubNotesCount(const double beatsPerSecond, const int availableDurationTicks)
{
    if (availableDurationTicks < MIN_TRILL_EVENT_TICKS * 2) {
        return 0;
    }

    const int eventCount = availableDurationTicks / boundedTrillTicksPerNote(beatsPerSecond);
    return eventCount >= 2 ? eventCount : 2;
}
...
void OrnamentsRenderer::convert(const ArticulationType type, const DisclosurePattern& pattern, NominalNoteCtx&& noteCtx,
                                mpe::PlaybackEventList& result)
{
    if (isBoundedTrillType(type)) {
        if (noteCtx.chordCtx.nominalDurationTicks < MIN_TRILL_EVENT_TICKS * 2) {
            result.emplace_back(buildNoteEvent(std::move(noteCtx)));
            return;
        }
    } else if (noteCtx.chordCtx.nominalDurationTicks <= pattern.boundaries.lowTempoDurationTicks) {
        result.emplace_back(buildNoteEvent(std::move(noteCtx)));
        return;
    }
    ...
    if (!pattern.alterationStepPitchOffsets.empty()) {
        ...
        if (pattern.isAlterationsRepeatAllowed) {
            if (isBoundedTrillType(type)) {
                alterationsCount = boundedTrillSubNotesCount(noteCtx.chordCtx.beatsPerSecond.val,
                                                             noteCtx.chordCtx.nominalDurationTicks - pattern.prefixDurationTicks
        ...
        if (alterationsCount == 0) {
            result.emplace_back(buildNoteEvent(std::move(noteCtx)));
        } else if (isBoundedTrillType(type)) {
            createTrillEvents(type, noteCtx, alterationsCount,
...
void OrnamentsRenderer::createTrillEvents(const ArticulationType type, NominalNoteCtx& noteCtx, const int subNotesCount,
...
{
    IF_ASSERT_FAILED(subNotesCount > 0 && !pitchOffsets.empty() && overallDurationTicks > 0) {
        return;
    }

    const float availableDurationRatio = availableDurationTicks / static_cast<float>(overallDurationTicks);
    const duration_t availableDuration = noteCtx.duration * availableDurationRatio;
    const timestamp_t bodyStartTimestamp = noteCtx.timestamp;

    track_idx_t trackIdx = staff2track(noteCtx.staffIdx, noteCtx.voiceIdx);

    for (int subNoteIdx = 0; subNoteIdx < subNotesCount; ++subNoteIdx) {
        const duration_t startOffset = (availableDuration * subNoteIdx) / subNotesCount;
        const duration_t endOffset = (availableDuration * (subNoteIdx + 1)) / subNotesCount;

        NominalNoteCtx subNoteCtx(noteCtx);
        subNoteCtx.timestamp = bodyStartTimestamp + startOffset;
        subNoteCtx.duration = endOffset - startOffset;
        subNoteCtx.pitchLevel += pitchOffsets.at(subNoteIdx % pitchOffsets.size());
    ...
    noteCtx.timestamp = bodyStartTimestamp + availableDuration;
}
