...
void TWrite::writeProperty(const EngravingItem* item, XmlWriter& xml, Pid pid, bool force)
{
    ...
    // Debug output for PIANOMANIA_HELD_NOTE - only when property is true
    if (pid == Pid::PIANOMANIA_HELD_NOTE && p.toBool()) {
        LOGI("*** PIANOMANIA_HELD_NOTE being written: value=true, force=%s ***", force ? "true" : "false");
        // Force write for PIANOMANIA_HELD_NOTE when it's true
        force = true;
    }
    ...
    // Debug output for PIANOMANIA_HELD_NOTE - only when property is true
    if (pid == Pid::PIANOMANIA_HELD_NOTE && p.toBool()) {
        LOGI("*** About to write PIANOMANIA_HELD_NOTE to XML: value=true, default=%s ***", d.isValid() ? (d.toBool() ? "true" : "false") : "invalid");
    }
