...
void TWrite::writeItem(const EngravingItem* item, XmlWriter& xml, WriteContext& ctx)
{
    ...
    case ElementType::WHAMMY_BAR:   write(item_cast<const WhammyBar*>(item), xml, ctx);
        break;
    case ElementType::RUBATO_ZONE:  write(item_cast<const RubatoZone*>(item), xml, ctx);
        break;
    case ElementType::PYRO_SPAN:    write(item_cast<const PyroSpan*>(item), xml, ctx);
        break;
    case ElementType::LASER_SPAN:   write(item_cast<const LaserSpan*>(item), xml, ctx);
        break;
    default: {
        UNREACHABLE;
...
void TWrite::writeProperty(const EngravingItem* item, XmlWriter& xml, Pid pid, bool force)
{
    ...
    // Pianomania Boolean note markers must always round-trip when set.
    if ((pid == Pid::PIANOMANIA_HELD_NOTE || pid == Pid::PIANOMANIA_HELD_NOTE_PULSE_TRIPLET
         || pid == Pid::PIANOMANIA_SHAKE_NOTE) && p.toBool()) {
        force = true;
    }
...
{
    writeProperties(toChordRest(item), xml, ctx);
    writeProperty(item, xml, Pid::PIANOMANIA_HAND);
}
...
void TWrite::write(const Note* item, XmlWriter& xml, WriteContext& ctx)
{
    ...
    for (Pid id : { Pid::PITCH, Pid::CENT_OFFSET, Pid::TPC1, Pid::TPC2, Pid::SMALL, Pid::MIRROR_HEAD, Pid::DOT_POSITION,
                    ...
                    Pid::GHOST, Pid::DEAD, Pid::HEAD_TYPE, Pid::FIXED, Pid::FIXED_LINE, Pid::PIANOMANIA_HELD_NOTE,
                    Pid::PIANOMANIA_HELD_NOTE_PULSE, Pid::PIANOMANIA_HELD_NOTE_PULSE_TRIPLET,
                    Pid::PIANOMANIA_HAND, Pid::PIANOMANIA_SHAKE_NOTE }) {
        writeProperty(item, xml, id);
    }
    if (!item->pianomaniaHeldNotePitchCurve().empty()) {
        xml.startElement("pianomaniaHeldNotePitchCurve", { { "version", 3 } });
        for (const PianomaniaHeldNotePitchCurvePoint& point : item->pianomaniaHeldNotePitchCurve()) {
            xml.tag("point", { { "scoreTick", point.scoreTick }, { "pitchCents", point.pitchCents },
                               { "slopeCentsPerQuarter", point.slopeCentsPerQuarter } });
        }
        xml.endElement();
    }
...
void TWrite::write(const RubatoZone* item, XmlWriter& xml, WriteContext& ctx)
{
    if (!ctx.canWrite(item)) {
        return;
    }
...
void TWrite::write(const PyroSpan* item, XmlWriter& xml, WriteContext& ctx)
{
    if (!ctx.canWrite(item)) {
        return;
    }
...
void TWrite::write(const LaserSpan* item, XmlWriter& xml, WriteContext& ctx)
{
    if (!ctx.canWrite(item)) {
        return;
    }
