...
void TRead::readItem(EngravingItem* item, XmlReader& xml, ReadContext& ctx)
{
    ...
    case ElementType::WHAMMY_BAR: read(item_cast<WhammyBar*>(item), xml, ctx);
        break;
    case ElementType::RUBATO_ZONE: read(item_cast<RubatoZone*>(item), xml, ctx);
        break;
    case ElementType::PYRO_SPAN: read(item_cast<PyroSpan*>(item), xml, ctx);
        break;
    case ElementType::LASER_SPAN: read(item_cast<LaserSpan*>(item), xml, ctx);
        break;
    default:
        UNREACHABLE;
...
bool TRead::readProperties(Note* n, XmlReader& e, ReadContext& ctx)
{
    ...
    if (tag == "pitch") {
        ...
        n->setLine(e.readInt());
    } else if (tag == "pianomaniaHeldNote") {
        n->setPianomaniaHeldNote(e.readBool());
    } else if (tag == "pianomaniaHeldNotePulse") {
        n->setPianomaniaHeldNotePulse(static_cast<Note::PianomaniaHeldNotePulse>(e.readInt()));
    } else if (tag == "pianomaniaHeldNotePulseTriplet") {
        n->setPianomaniaHeldNotePulseTriplet(e.readBool());
    } else if (tag == "pianomaniaHeldNotePitchCurve") {
        ctx.pianomaniaHeldNotePitchCurveMigration().read(n, e);
    } else if (tag == "pianomaniaShakeNote") {
        n->setPianomaniaShakeNote(e.readBool());
    } else if (tag == "pianomaniaHand") {
        TRead::readProperty(n, e, ctx, Pid::PIANOMANIA_HAND);
...
void TRead::read(Rest* r, XmlReader& e, ReadContext& ctx)
{
    while (e.readNextStartElement()) {
        ...
        if (tag == "NoteDot") {
            ...
            TRead::readProperty(r, e, ctx, Pid::PIANOMANIA_HAND);
        } else if (TRead::readStyledProperty(r, tag, e, ctx)) {
    ...
    if (ctx.pasteMode()) {
        r->assignPianomaniaHandFromStaff();
    }
}
