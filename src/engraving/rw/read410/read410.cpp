...
bool Read410::readScoreTag(Score* score, XmlReader& e, ReadContext& ctx)
{
    ...
    score->connectTies();
    muse::String pitchCurveMigrationError;
    if (!ctx.pianomaniaHeldNotePitchCurveMigration().migrate(score, &pitchCurveMigrationError)) {
        e.raiseError(pitchCurveMigrationError);
        LOGE() << pitchCurveMigrationError;
        return false;
    }
