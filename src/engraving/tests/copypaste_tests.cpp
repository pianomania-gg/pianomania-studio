...
TEST_F(Engraving_CopyPasteTests, PianomaniaHandFollowsDestinationStaff)
{
    ...
    ASSERT_TRUE(measure);
    score->select(measure, SelectType::RANGE, 0);
    ...
    score->startCmd(TranslatableString::untranslatable("Paste Pianomania hand test"));
    QMimeDataAdapter mimeDataAdapter(&mimeData);
    score->cmdPaste(&mimeDataAdapter, 0);
    score->endCmd();

    Chord* pastedChord = measure->findChord(Fraction(0, 1), 4);
    ASSERT_TRUE(pastedChord);
    ASSERT_FALSE(pastedChord->notes().empty());
    EXPECT_TRUE(std::all_of(pastedChord->notes().cbegin(), pastedChord->notes().cend(), [](const Note* note) {
        return note->pianomaniaHand() == Note::PianomaniaHand::Left;
    }));
}
