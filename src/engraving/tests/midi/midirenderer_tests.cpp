...
static constexpr int NOTE_OFF_VOLUME = 64;
                               ...
                               int channel = DEFAULT_CHANNEL)
{
    ...
    EXPECT_EQ(it->second.type(), EventType::ME_NOTEON);
    ...
    EXPECT_EQ(it->second.type(), EventType::ME_NOTEOFF);
    EXPECT_EQ(it->second.channel(), channel);
    EXPECT_EQ(it->second.effect(), effect);
...
TEST_F(MidiRenderer_Tests, mergePitchWheelEvents)
{
    ...
    NPlayEvent note1_ON{ EventType::ME_NOTEON, 0, 59, 96 };
    NPlayEvent note1_OFF{ EventType::ME_NOTEOFF, 0, 59, NOTE_OFF_VOLUME };
    NPlayEvent note2_ON{ EventType::ME_NOTEON, 0, 61, 96 };
    NPlayEvent note2_OFF{ EventType::ME_NOTEOFF, 0, 61, NOTE_OFF_VOLUME };
...
TEST_F(MidiRenderer_Tests, invisibleOneGuitarNoteUsesOrnamentNoteOffVelocity)
{
    constexpr int defVol = 96; // f
    constexpr int ornamentNoteOffVelocity = 127;

    MasterScore* score = ScoreRW::readScore(MIDIRENDERER_TESTS_DIR + u"one_guitar_note.mscx");
    ASSERT_TRUE(score);

    Note* firstNote = nullptr;
    for (Segment* segment = score->firstSegment(SegmentType::ChordRest); segment && !firstNote; segment = segment->next1()) {
        for (track_idx_t track = 0; track < score->ntracks() && !firstNote; ++track) {
            ...
            if (!chord->notes().empty()) {
                firstNote = chord->notes().front();
            }
        }
    }
    ASSERT_TRUE(firstNote);

    firstNote->setVisible(false);
    ...
    auto noteOnIt = events[DEFAULT_CHANNEL].find(0);
    ASSERT_NE(noteOnIt, events[DEFAULT_CHANNEL].end());
    EXPECT_EQ(noteOnIt->second.type(), EventType::ME_NOTEON);
    EXPECT_EQ(noteOnIt->second.pitch(), 59);
    EXPECT_EQ(noteOnIt->second.velo(), defVol);

    auto noteOffIt = events[DEFAULT_CHANNEL].find(479);
    ASSERT_NE(noteOffIt, events[DEFAULT_CHANNEL].end());
