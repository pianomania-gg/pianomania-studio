...
TEST_F(PitchWheelRender_Tests, adjacentPianomaniaRangesResetBeforeNextSensitivity)
{
    PitchWheelSpecs wheelSpec;
    PitchWheelRenderer render(wheelSpec);

    PitchWheelRenderer::PitchWheelFunction first;
    first.mStartTick = 0;
    first.mEndTick = 30;
    first.func = [](uint32_t) { return 100; };
    render.addPianomaniaPitchWheelFunction(first, 0, 0, MidiInstrumentEffect::NONE, "first", 2);

    PitchWheelRenderer::PitchWheelFunction second;
    second.mStartTick = 30;
    second.mEndTick = 60;
    second.func = [](uint32_t) { return -100; };
    render.addPianomaniaPitchWheelFunction(second, 0, 0, MidiInstrumentEffect::NONE, "second", 3);

    EventsHolder events = render.renderPitchWheel();
    EXPECT_EQ(events[DEFAULT_CHANNEL].count(29), 0u);
    const auto boundaryEvents = events[DEFAULT_CHANNEL].equal_range(30);
    ASSERT_NE(boundaryEvents.first, boundaryEvents.second);
    auto nextIt = boundaryEvents.first;
    ASSERT_EQ(nextIt->second.type(), EventType::ME_PITCHBEND);
    EXPECT_EQ(pitch(nextIt->second), 8192 + 100);
    ++nextIt;
    ASSERT_NE(nextIt, boundaryEvents.second);
    ASSERT_EQ(nextIt->second.type(), EventType::ME_PITCHBEND);
    EXPECT_EQ(pitch(nextIt->second), 8192);
    ++nextIt;
    ASSERT_NE(nextIt, boundaryEvents.second);
    EXPECT_EQ(nextIt->second.type(), EventType::ME_CONTROLLER);
    EXPECT_EQ(nextIt->second.dataA(), CTRL_HRPN);
    ++nextIt;
    ASSERT_NE(nextIt, boundaryEvents.second);
    EXPECT_EQ(nextIt->second.type(), EventType::ME_CONTROLLER);
    EXPECT_EQ(nextIt->second.dataA(), CTRL_LRPN);
    ++nextIt;
    ASSERT_NE(nextIt, boundaryEvents.second);
    EXPECT_EQ(nextIt->second.type(), EventType::ME_CONTROLLER);
    EXPECT_EQ(nextIt->second.dataA(), CTRL_HDATA);
    EXPECT_EQ(nextIt->second.dataB(), 3);
    std::advance(nextIt, 4);
    ASSERT_NE(nextIt, boundaryEvents.second);
    EXPECT_EQ(nextIt->second.type(), EventType::ME_PITCHBEND);
    EXPECT_EQ(pitch(nextIt->second), 8192 - 100);
    ++nextIt;
    EXPECT_EQ(nextIt, boundaryEvents.second);
}
