...
TEST_F(Engraving_SelectionRangeTests, addPyroSpanUsesSelectedTicksAndStaff)
{
    MasterScore* score = ScoreRW::readScore(SELRANGE_DATA_DIR + u"pyrospanstaffindependent.mscx");
    ASSERT_TRUE(score);

    Measure* firstMeasure = score->firstMeasure();
    Measure* secondMeasure = firstMeasure->nextMeasure();
    Measure* thirdMeasure = secondMeasure->nextMeasure();
    ASSERT_TRUE(firstMeasure);
    ASSERT_TRUE(secondMeasure);
    ASSERT_TRUE(thirdMeasure);

    Segment* betweenOnsets = firstMeasure->findSegment(SegmentType::ChordRest, Fraction(1, 4));
    Segment* secondMeasureStart = secondMeasure->first(SegmentType::ChordRest);
    ASSERT_TRUE(betweenOnsets);
    ASSERT_TRUE(secondMeasureStart);

    score->selection().setRange(betweenOnsets, secondMeasureStart, 0, 1);
    ASSERT_TRUE(score->cmdAddPyroSpan());

    Spanner* betweenOnsetSpan = nullptr;
    for (const auto& interval : score->spannerMap().findOverlapping(0, score->endTick().ticks())) {
        if (interval.value && interval.value->isPyroSpan()) {
            betweenOnsetSpan = interval.value;
        }
    }
    ASSERT_TRUE(betweenOnsetSpan);
    EXPECT_EQ(betweenOnsetSpan->tick(), Fraction(1, 4));
    EXPECT_EQ(betweenOnsetSpan->tick2(), Fraction(1, 1));
    EXPECT_EQ(betweenOnsetSpan->track(), 0);

    score->selection().setRange(secondMeasureStart, nullptr, 0, 1);
    ASSERT_TRUE(score->cmdAddPyroSpan());

    Spanner* restMeasureSpan = nullptr;
    size_t pyroSpanCount = 0;
    for (const auto& interval : score->spannerMap().findOverlapping(0, score->endTick().ticks())) {
        if (!interval.value || !interval.value->isPyroSpan()) {
            continue;
        }
        ++pyroSpanCount;
        if (interval.value->tick() == secondMeasure->tick()) {
            restMeasureSpan = interval.value;
        }
    }
    ASSERT_EQ(pyroSpanCount, 2u);
    ASSERT_TRUE(restMeasureSpan);
    EXPECT_EQ(restMeasureSpan->tick(), secondMeasure->tick());
    EXPECT_EQ(restMeasureSpan->tick2(), thirdMeasure->endTick());
    EXPECT_EQ(restMeasureSpan->track(), 0);

    score->selection().setRange(secondMeasureStart, thirdMeasure->first(SegmentType::ChordRest), 0, 1);
    EXPECT_FALSE(score->cmdAddPyroSpan());

    delete score;
}

TEST_F(Engraving_SelectionRangeTests, addLaserSpanFindsNoteEndpointsAcrossSelectedStaves)
{
    MasterScore* score = ScoreRW::readScore(SELRANGE_DATA_DIR + u"pyrospanstaffindependent.mscx");
    ASSERT_TRUE(score);
    ...
    score->selection().setRange(firstMeasure->first(SegmentType::ChordRest), nullptr, 0, 2);
    ASSERT_TRUE(score->cmdAddLaserSpan());

    Spanner* laserSpan = nullptr;
    for (const auto& interval : score->spannerMap().findOverlapping(0, score->endTick().ticks())) {
        if (interval.value && interval.value->isLaserSpan()) {
            laserSpan = interval.value;
        }
    }
    ASSERT_TRUE(laserSpan);
    EXPECT_EQ(laserSpan->tick(), Fraction(0, 1));
    EXPECT_EQ(laserSpan->tick2(), score->endTick());
    EXPECT_EQ(laserSpan->track(), staff2track(1));

    score->selection().setRange(firstMeasure->first(SegmentType::ChordRest), nullptr, 0, 2);
    EXPECT_FALSE(score->cmdAddLaserSpan());

    delete score;
}

TEST_F(Engraving_SelectionRangeTests, addLaserSpanRejectsRestOnlyEndpoints)
{
    MasterScore* score = ScoreRW::readScore(SELRANGE_DATA_DIR + u"pyrospanstaffindependent.mscx");
    ASSERT_TRUE(score);
    ...
    score->selection().setRange(firstMeasure->first(SegmentType::ChordRest), nullptr, 0, 1);
    EXPECT_FALSE(score->cmdAddLaserSpan());
