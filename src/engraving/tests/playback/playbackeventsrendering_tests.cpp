...
static void setPlaybackTempo(Score* score, const BeatsPerSecond bps)
{
    score->tempomap()->setTempo(0, bps);
...
TEST_F(Engraving_PlaybackEventsRendererTests, PianomaniaHeldNotePitchCurveRendersForPlayback)
{
    Score* score = ScoreRW::readScore(PLAYBACK_EVENTS_RENDERING_DIR + "single_note_no_articulations/no_articulations.mscx");
    Measure* firstMeasure = score->firstMeasure();
    ASSERT_TRUE(firstMeasure);
    Segment* firstSegment = firstMeasure->segments().firstCRSegment();
    ASSERT_TRUE(firstSegment);
    ChordRest* chordRest = firstSegment->nextChordRest(0);
    ASSERT_TRUE(chordRest && chordRest->isChord());
    ...
    const PianomaniaHeldNotePitchCurve curve {
        { 0, 0, 400 }, { durationTicks / 2, 200, 0 }, { durationTicks, 0, 0 }
    };
    note->setPianomaniaHeldNote(true);
    EXPECT_TRUE(note->setProperty(Pid::PIANOMANIA_HELD_NOTE_PITCH_CURVE, curve));

    m_defaultProfile->setPattern(ArticulationType::Standard, m_dummyPattern);
    PlaybackEventsMap result;
    m_renderer.render(chord, 0, m_defaultProfile, std::make_shared<PlaybackContext>(), result);

    ASSERT_EQ(result.size(), 1);
    ASSERT_EQ(result.begin()->second.size(), 1u);
    const mpe::NoteEvent& event = std::get<mpe::NoteEvent>(result.begin()->second.front());
    const mpe::PitchCurve& playbackCurve = event.pitchCtx().pitchCurve;
    ASSERT_FALSE(playbackCurve.empty());
    EXPECT_EQ(playbackCurve.at(0), 0);
    EXPECT_EQ(playbackCurve.at(mpe::FIFTY_PERCENT / 2), 63);
    EXPECT_EQ(playbackCurve.at(mpe::FIFTY_PERCENT), 100);
    EXPECT_EQ(playbackCurve.at(mpe::HUNDRED_PERCENT), 0);
    const auto multibend = event.expressionCtx().articulations.find(mpe::ArticulationType::Multibend);
    ASSERT_NE(multibend, event.expressionCtx().articulations.cend());
    EXPECT_EQ(multibend->second.meta.timestamp, event.arrangementCtx().actualTimestamp);
    EXPECT_EQ(multibend->second.meta.overallDuration, event.arrangementCtx().actualDuration);
...
TEST_F(Engraving_PlaybackEventsRendererTests, SingleNote_Trill_Modern)
{
    ...
    // [GIVEN] Expected bounded trill disclosure
    constexpr size_t expectedTrillSubNotesCount = 6;
    const pitch_level_t principalPitch = pitchLevel(PitchClass::F, 4);
    const pitch_level_t upperPitch = principalPitch + 2 * PITCH_LEVEL_STEP;
    ...
    for (const auto& pair : result) {
        ...
        for (size_t i = 0; i < pair.second.size(); ++i) {
            ...
            // [THEN] Modern trills alternate from the principal note to the upper note
            const pitch_level_t expectedPitch = i % 2 == 0 ? principalPitch : upperPitch;
            EXPECT_EQ(noteEvent.pitchCtx().nominalPitchLevel, expectedPitch);
            EXPECT_GT(noteEvent.arrangementCtx().actualDuration, 0);
        }
    }
}

TEST_F(Engraving_PlaybackEventsRendererTests, SingleNote_Trill_FastTempoIsBounded)
{
    Score* score = ScoreRW::readScore(
        PLAYBACK_EVENTS_RENDERING_DIR + "single_note_trill_default_tempo/single_note_trill_default_tempo.mscx");
    setPlaybackTempo(score, BeatsPerSecond(4.0)); // 240 bpm
    ...
    ASSERT_EQ(result.size(), 1);
    const PlaybackEventList& events = result.begin()->second;
    ASSERT_EQ(events.size(), 3);

    constexpr duration_t fastestUsefulTrillDuration = 1000000 / 16;
    for (const PlaybackEvent& event : events) {
        ASSERT_TRUE(std::holds_alternative<mpe::NoteEvent>(event));
        const mpe::NoteEvent& noteEvent = std::get<mpe::NoteEvent>(event);
        EXPECT_GE(noteEvent.arrangementCtx().actualDuration, fastestUsefulTrillDuration);
    }

    delete score;
}

TEST_F(Engraving_PlaybackEventsRendererTests, SingleNote_Trill_SlowTempoStaysReadable)
{
    Score* score = ScoreRW::readScore(
        PLAYBACK_EVENTS_RENDERING_DIR + "single_note_trill_default_tempo/single_note_trill_default_tempo.mscx");
    setPlaybackTempo(score, BeatsPerSecond(1.0)); // 60 bpm
    ...
    constexpr duration_t slowestUsefulTrillDuration = 1000000 / 8;
    for (const PlaybackEvent& event : events) {
        ASSERT_TRUE(std::holds_alternative<mpe::NoteEvent>(event));
        const mpe::NoteEvent& noteEvent = std::get<mpe::NoteEvent>(event);
        EXPECT_LE(noteEvent.arrangementCtx().actualDuration, slowestUsefulTrillDuration);
    }
...
/**
 * @brief PlaybackEventsRendererTests_SingleNote_Trill_MinimumBoundedDisclosure
 * @details In this case we're gonna render a simple piece of score with a single measure,
 *          which starts with the F4 32-nd note marked by Trill articulation.
 */
TEST_F(Engraving_PlaybackEventsRendererTests, SingleNote_Trill_MinimumBoundedDisclosure)
{
    // [GIVEN] Simple piece of score (piano, 4/4, 120 bpm, Treble Cleff)
    Score* score = ScoreRW::readScore(
    ...
    // [GIVEN] Expected minimum bounded trill disclosure
    constexpr size_t expectedTrillSubNotesCount = 2;
    ...
    // [THEN] We expect the short trill to disclose only when it can keep both subnotes above the tick floor
    ASSERT_EQ(result.size(), 1);
    EXPECT_EQ(result.begin()->second.size(), expectedTrillSubNotesCount);
}
...
TEST_F(Engraving_PlaybackEventsRendererTests, SingleNote_Trill_Baroque)
{
    ...
    for (const auto& pair : result) {
        ...
        for (size_t i = 0; i < pair.second.size(); ++i) {
            ...
            // [THEN] Baroque trill playback starts on the upper note and alternates back to the principal note
            const pitch_level_t expectedPitch = i % 2 == 0 ? upperPitch : principalPitch;
            EXPECT_EQ(noteEvent.pitchCtx().nominalPitchLevel, expectedPitch);
            EXPECT_GT(noteEvent.arrangementCtx().actualDuration, 0);
        }
    }
    ...
    for (const auto& pair : result) {
        for (const PlaybackEvent& event : pair.second) {
            ...
            if (noteNum == 0 || noteNum == 3) {
                ...
                constexpr size_t expectedSubNotes = 24;
                ...
                EXPECT_GE(noteEvent.arrangementCtx().actualDuration, expectedSubNoteDuration);
                EXPECT_LE(noteEvent.arrangementCtx().actualDuration, expectedSubNoteDuration + 1);
            }
        }
...
TEST_F(Engraving_PlaybackEventsRendererTests, Trill_TiedNotes)
{
    ...
    for (const auto& pair : result) {
        ...
        constexpr size_t expectedSubNotes = 48;
        EXPECT_EQ(pair.second.size(), expectedSubNotes);
