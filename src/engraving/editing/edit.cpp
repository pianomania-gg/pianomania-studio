...
#include "../dom/rubatozone.h"
#include "../dom/pyrospan.h"
#include "../dom/laserspan.h"
#include "../dom/part.h"
...
//   cmdAddRubatoZone
//
//   Pianomania: bracket the selected note range as a rubato
//   zone. One zone per selection; anchored to the first and
//   last selected ChordRests.
//---------------------------------------------------------

bool Score::cmdAddRubatoZone()
{
    const Selection& selectedRange = selection();
    Measure* startMeasure = nullptr;
    Measure* endMeasure = nullptr;
    if (!selectedRange.isRange()
        || !selectedRange.measureRange(&startMeasure, &endMeasure)
        || !startMeasure
        || !endMeasure
        || selectedRange.tickStart() != startMeasure->tick()
        || selectedRange.tickEnd() != endMeasure->endTick()) {
        return false;
    }

    ChordRest* cr1 = selectedRange.firstChordRest();
    ChordRest* cr2 = selectedRange.lastChordRest();
    if (!cr1 || !cr2) {
        return false;
    }

    RubatoZone* zone = Factory::createRubatoZone(this->dummy());
    zone->setTrack(cr1->track());
    zone->setTrack2(cr1->track());
    zone->setTick(startMeasure->tick());
    zone->setTick2(endMeasure->endTick());
    undoAddElement(zone);
    if (!noteEntryMode()) {
        select(zone, SelectType::SINGLE, 0);
    }
    return true;
}
...
//   cmdAddDanceShowSpan
//
//   Pianomania: bracket the selected score-time range as a dance-show
//   effect span (pyro or laser). Pyro uses the range selection's exact
//   ticks and selected staff. Laser retains chord-only endpoints. Spans
//   of the same type must not overlap.
//---------------------------------------------------------

static ChordRest* chordAtSelectedRangeBoundary(const Selection& selection, bool start)
{
    ChordRest* boundaryChordRest = start ? selection.firstChordRest() : selection.lastChordRest();
    if (!boundaryChordRest) {
        return nullptr;
    }

    Segment* boundarySegment = boundaryChordRest->segment();
    for (track_idx_t track = staff2track(selection.staffStart()); track < staff2track(selection.staffEnd()); ++track) {
        EngravingItem* item = boundarySegment->element(track);
...
static bool cmdAddDanceShowSpan(Score* score, ElementType spanType)
{
    const Selection& selectedRange = score->selection();
    if (!selectedRange.isRange()) {
        return false;
    }

    const bool isPyro = spanType == ElementType::PYRO_SPAN;
    ChordRest* cr1 = isPyro
                     ? selectedRange.firstChordRest()
                     : chordAtSelectedRangeBoundary(selectedRange, true);
    ChordRest* cr2 = isPyro
                     ? selectedRange.lastChordRest()
                     : chordAtSelectedRangeBoundary(selectedRange, false);
    if (!isPyro && (!cr1 || !cr2)) {
        return false;
    }

    const Fraction tick = isPyro ? selectedRange.tickStart() : cr1->tick();
    const Fraction tick2 = isPyro ? selectedRange.tickEnd() : cr2->tick() + cr2->actualTicks();
    if (tick < Fraction(0, 1) || tick2 <= tick) {
        return false;
    }
    for (const auto& pair : score->spannerMap().map()) {
        const Spanner* existing = pair.second;
        if (existing->type() != spanType) {
            continue;
        }
        if (existing->tick() < tick2 && tick < existing->tick2()) {
            return false;
        }
    }

    Spanner* span = isPyro
                    ? static_cast<Spanner*>(Factory::createPyroSpan(score->dummy()))
                    : static_cast<Spanner*>(Factory::createLaserSpan(score->dummy()));
    const track_idx_t track = isPyro ? staff2track(selectedRange.staffStart()) : cr1->track();
    span->setTrack(track);
    span->setTrack2(track);
    span->setTick(tick);
    span->setTick2(tick2);
    score->undoAddElement(span);
    if (!score->noteEntryMode()) {
        score->select(span, SelectType::SINGLE, 0);
    }
    return true;
}

bool Score::cmdAddPyroSpan()
{
    return cmdAddDanceShowSpan(this, ElementType::PYRO_SPAN);
}

bool Score::cmdAddLaserSpan()
{
    return cmdAddDanceShowSpan(this, ElementType::LASER_SPAN);
}
        ...
        if (e->isBeam()) {
            ...
            flipOnce(ornament, [ornament]() {
                ...
                ornament->undoChangeProperty(Pid::ARTICULATION_ANCHOR, int(articAnchor), pf);
                   ...
                   || e->isRubatoZone()
                   || e->isRubatoZoneSegment()
                   || e->isPyroSpan()
                   || e->isPyroSpanSegment()
                   || e->isLaserSpan()
                   || e->isLaserSpanSegment()
                   || e->isRasgueado()
...
//      LOGD("%s", el->typeName());
    ...
    case ElementType::PYRO_SPAN_SEGMENT:
    case ElementType::LASER_SPAN_SEGMENT:
