...
void Score::addInterval(int val, const std::vector<Note*>& nl)
{
    ...
    for (Note* on : tmpnl) {
        ...
        note->assignPianomaniaHandFromStaff();
...
//---------------------------------------------------------
//   togglePianomaniaHeldNotes
//---------------------------------------------------------

static std::vector<std::vector<Note*> > pianomaniaTieChains(const std::vector<Note*>& notes)
{
    std::set<Note*> handledNotes;
    std::vector<std::vector<Note*>> tieChains;

    for (Note* selectedNote : notes) {
        if (!selectedNote || handledNotes.find(selectedNote) != handledNotes.cend()) {
            continue;
        }

        std::vector<Note*> tieChain = selectedNote->tiedNotes();
        for (Note* note : tieChain) {
            if (note) {
                handledNotes.insert(note);
            }
        }
        tieChains.push_back(std::move(tieChain));
    }

    return tieChains;
}

void Score::togglePianomaniaHeldNotes(const std::vector<Note*>& notes)
{
    const std::vector<std::vector<Note*> > tieChains = pianomaniaTieChains(notes);
    const bool allHeld = !tieChains.empty() && std::all_of(tieChains.cbegin(), tieChains.cend(), [](const std::vector<Note*>& tieChain) {
        return !tieChain.empty() && std::all_of(tieChain.cbegin(), tieChain.cend(), [](const Note* note) {
            return note && note->pianomaniaHeldNote();
        });
    });

    for (const std::vector<Note*>& tieChain : tieChains) {
        for (Note* note : tieChain) {
            if (note) {
                if (allHeld) {
                    note->undoChangeProperty(Pid::PIANOMANIA_HELD_NOTE_PITCH_CURVE, PianomaniaHeldNotePitchCurve(), PropertyFlags::UNSTYLED);
                    note->undoChangeProperty(Pid::PIANOMANIA_HELD_NOTE_PULSE_TRIPLET, false, PropertyFlags::UNSTYLED);
                    note->undoChangeProperty(Pid::PIANOMANIA_HELD_NOTE_PULSE,
                                             static_cast<int>(Note::PianomaniaHeldNotePulse::None),
                                             PropertyFlags::UNSTYLED);
                }
                note->undoChangeProperty(Pid::PIANOMANIA_HELD_NOTE, !allHeld, PropertyFlags::UNSTYLED);
            }
        }
    }
}
...
//---------------------------------------------------------
//   setPianomaniaHeldNotePulse
//---------------------------------------------------------

void Score::setPianomaniaHeldNotePulse(const std::vector<Note*>& notes, int pulseValue)
{
    const Note::PianomaniaHeldNotePulse pulse = static_cast<Note::PianomaniaHeldNotePulse>(pulseValue);
    for (const std::vector<Note*>& tieChain : pianomaniaTieChains(notes)) {
        for (Note* note : tieChain) {
            if (!note) {
                continue;
            }
            if (pulse == Note::PianomaniaHeldNotePulse::None) {
                note->undoChangeProperty(Pid::PIANOMANIA_HELD_NOTE_PULSE_TRIPLET, false, PropertyFlags::UNSTYLED);
            }
            note->undoChangeProperty(Pid::PIANOMANIA_HELD_NOTE_PULSE, pulseValue, PropertyFlags::UNSTYLED);
        }
    }
}

//---------------------------------------------------------
//   setPianomaniaHeldNotePulseTriplet
//---------------------------------------------------------

void Score::setPianomaniaHeldNotePulseTriplet(const std::vector<Note*>& notes, bool triplet)
{
    for (const std::vector<Note*>& tieChain : pianomaniaTieChains(notes)) {
        for (Note* note : tieChain) {
            if (note) {
                note->undoChangeProperty(Pid::PIANOMANIA_HELD_NOTE_PULSE_TRIPLET, triplet, PropertyFlags::UNSTYLED);
            }
        }
    }
}
...
//---------------------------------------------------------
//   setPianomaniaHeldNotePitchCurve
//---------------------------------------------------------

bool Score::setPianomaniaHeldNotePitchCurve(const std::vector<Note*>& notes, const PianomaniaHeldNotePitchCurve& curve)
{
    if (!Note::isValidPianomaniaHeldNotePitchCurve(curve)) {
        return false;
    }

    for (const std::vector<Note*>& tieChain : pianomaniaTieChains(notes)) {
        const int durationTicks = tieChain.empty() || !tieChain.front() ? 0
                                                                       : tieChain.front()->pianomaniaHeldNoteDurationTicks();
        if (!curve.empty() && !Note::isValidPianomaniaHeldNotePitchCurve(curve, durationTicks)) {
            return false;
        }
        for (Note* note : tieChain) {
            if (!note || !note->pianomaniaHeldNote()) {
                return false;
            }
        }
    }
    for (const std::vector<Note*>& tieChain : pianomaniaTieChains(notes)) {
        for (Note* note : tieChain) {
            note->undoChangeProperty(Pid::PIANOMANIA_HELD_NOTE_PITCH_CURVE, curve, PropertyFlags::UNSTYLED);
        }
    }
    return true;
}
...
//---------------------------------------------------------
//   togglePianomaniaShakeNotes
//---------------------------------------------------------

void Score::togglePianomaniaShakeNotes(const std::vector<Note*>& notes)
{
    std::set<Note*> handledNotes;
    std::vector<std::vector<Note*>> tieChains;
    ...
    for (const std::vector<Note*>& tieChain : tieChains) {
        const bool allShake = std::all_of(tieChain.cbegin(), tieChain.cend(), [](const Note* note) {
            return note && note->pianomaniaShakeNote();
        });

        for (Note* note : tieChain) {
            if (note) {
                note->undoChangeProperty(Pid::PIANOMANIA_SHAKE_NOTE, !allShake, PropertyFlags::UNSTYLED);
            }
        }
    }
}
...
{
    if (attr == SymId::ornamentTurn) {
        std::set<Chord*> chordSet;
        std::vector<Chord*> chordList;
        for (EngravingItem* el : selection().elements()) {
            Chord* chord = nullptr;
            if (el->isNote()) {
                chord = toNote(el)->chord();
            } else if (el->isChord()) {
                chord = toChord(el);
            }

            if (chord && chordSet.insert(chord).second) {
                chordList.push_back(chord);
            }
        }

        if (chordList.size() == 2) {
            std::sort(chordList.begin(), chordList.end(), [](const Chord* left, const Chord* right) {
            ...
            Chord* startChord = chordList.front();
            Chord* endChord = chordList.back();
            if (startChord->staffIdx() == endChord->staffIdx() && startChord->segment() && endChord->segment()) {
                Trill* trill = Factory::createTrill(this->dummy()->chord());
                trill->setTrillType(TrillType::TURN_BETWEEN);
                cmdAddSpanner(trill, startChord->staffIdx(), startChord->segment(), endChord->segment(), false);
                return;
            }
        }
    }
