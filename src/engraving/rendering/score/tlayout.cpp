...
void TLayout::layoutItem(EngravingItem* item, LayoutContext& ctx)
{
    ...
    case ElementType::WHAMMY_BAR_SEGMENT: layoutWhammyBarSegment(item_cast<WhammyBarSegment*>(item), ctx);
        break;
    case ElementType::RUBATO_ZONE_SEGMENT: layoutRubatoZoneSegment(item_cast<RubatoZoneSegment*>(item), ctx);
        break;
    case ElementType::PYRO_SPAN_SEGMENT: layoutPyroSpanSegment(item_cast<PyroSpanSegment*>(item), ctx);
        break;
    case ElementType::LASER_SPAN_SEGMENT: layoutLaserSpanSegment(item_cast<LaserSpanSegment*>(item), ctx);
        break;
    default:
...
void TLayout::layoutTrillSegment(TrillSegment* item, LayoutContext& ctx)
{
    ...
    if (item->isSingleType() || item->isBeginType()) {
        ...
        case TrillType::DOWNPRALL_LINE:
            item->symbolLine(SymId::ornamentLeftVerticalStroke,
                             SymId::ornamentZigZagLineNoRightEnd, SymId::ornamentZigZagLineWithRightEnd);
            break;
        case TrillType::TURN_BETWEEN:
            item->symbolLine(SymId::ornamentTurn, SymId::ornamentTurn);
            break;
        }
    ...
    if (trill->trillType() == TrillType::TURN_BETWEEN && item->isSingleType() && !item->symbols().empty()) {
        std::shared_ptr<const IEngravingFont> font = ctx.conf().engravingFont();
        double symbolWidth = font->advance(item->symbols().front(), item->magS());
        double midX = item->pos().x() + item->pos2().x() * 0.5;
        item->setPos(PointF(midX - symbolWidth * 0.5, item->pos().y()));
        item->setPos2(PointF(symbolWidth, 0.0));
    }
...
void TLayout::layoutRubatoZoneSegment(RubatoZoneSegment* item, LayoutContext& ctx)
{
    LAYOUT_CALL_ITEM(item);
    RubatoZoneSegment::LayoutData* ldata = item->mutldata();
    layoutTextLineBaseSegment(item, ctx);

    Autoplace::autoplaceSpannerSegment(item, ldata, ctx.conf().spatium());
}

void TLayout::layoutPyroSpanSegment(PyroSpanSegment* item, LayoutContext& ctx)
{
    LAYOUT_CALL_ITEM(item);
    PyroSpanSegment::LayoutData* ldata = item->mutldata();
    layoutTextLineBaseSegment(item, ctx);

    Autoplace::autoplaceSpannerSegment(item, ldata, ctx.conf().spatium());
}

void TLayout::layoutLaserSpanSegment(LaserSpanSegment* item, LayoutContext& ctx)
{
    LAYOUT_CALL_ITEM(item);
    LaserSpanSegment::LayoutData* ldata = item->mutldata();
    layoutTextLineBaseSegment(item, ctx);
