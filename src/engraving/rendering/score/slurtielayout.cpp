...
{
    return slurSeg->isEdited() || !slurSeg->offset().isNull() || !slurSeg->autoplace();
}

bool isPianomaniaLocalSingleSystemCrossStaff(const SlurSegment* slurSeg, const ChordRest* startCR, const ChordRest* endCR)
{
    if (!slurSeg || !slurSeg->slur() || !startCR || !endCR || !slurSeg->slur()->isCrossStaff() || !slurSeg->isSingleType()) {
        return false;
    }

    const Measure* startMeasure = startCR->measure();
    const Measure* endMeasure = endCR->measure();
    return startMeasure && endMeasure && startMeasure->system() && startMeasure->system() == endMeasure->system();
}

bool keepPianomaniaLocalCrossStaffCollisionShape(const ShapeElement& shapeEl)
{
...
void forceNormalizeManualSlurIfRequested(SlurSegment* slurSeg, bool manualEdit)
{
    if (!manualEdit || !MScore::pianomaniaForceNormalizeSlursFingerings) {
        return;
    }

    slurSeg->undoChangeProperty(Pid::SLUR_UOFF1, PointF(), slurSeg->propertyFlags(Pid::SLUR_UOFF1));
    slurSeg->undoChangeProperty(Pid::SLUR_UOFF2, PointF(), slurSeg->propertyFlags(Pid::SLUR_UOFF2));
    slurSeg->undoChangeProperty(Pid::SLUR_UOFF3, PointF(), slurSeg->propertyFlags(Pid::SLUR_UOFF3));
    slurSeg->undoChangeProperty(Pid::SLUR_UOFF4, PointF(), slurSeg->propertyFlags(Pid::SLUR_UOFF4));
    slurSeg->undoChangeProperty(Pid::OFFSET, PointF(), PropertyFlags::STYLED);
    slurSeg->undoChangeProperty(Pid::AUTOPLACE, true, slurSeg->propertyFlags(Pid::AUTOPLACE));
    ++MScore::pianomaniaNormalizedManualSlurs;
}

// Pianomania slur shaping.
//
// The pipeline shapes the slur side MuseScore already selected. It never flips a slur.
// Order of operations inside computeBezier:
//   1. classifyPianomaniaSlur       — explicit segment classes (short steep cross-staff,
//                                     local cross-staff, broken continuation, long phrase, default)
//   2. raisePianomaniaBuriedEndpoints — bounded endpoint lift when a cross-staff anchor sits so far
//                                     below (or above, for down slurs) the obstacle envelope that no
//                                     smooth arc can reach it
//   3. pianomaniaBaseShoulderHeight — flattest acceptable default arc; collision avoidance may only
//                                     raise it, so flattening can never re-introduce collisions
//   4. avoidCollisions              — MuseScore's structural solver, fed with noteheads, stems,
//                                     beams, flags/hooks, clefs and accidentals of both staves
//   5. reconcilePianomaniaSlurShape — smoothness invariants (no S-curves, hooks or lopsided arcs)
//                                     and a collision-checked cosmetic height cap
enum class PmSlurClass {
    Default,
    LongPhrase,
    BrokenSegment,
    LocalCrossStaff,
    ShortSteepCrossStaff,
};

PmSlurClass classifyPianomaniaSlur(const SlurSegment* slurSeg, const ChordRest* startCR, const ChordRest* endCR,
                                   double spanSp, double endpointDeltaSp)
{
    if (!slurSeg->isSingleType()) {
        return PmSlurClass::BrokenSegment;
    }
    if (isPianomaniaLocalSingleSystemCrossStaff(slurSeg, startCR, endCR)) {
        if (spanSp < 10.0 && endpointDeltaSp > 1.5) {
            return PmSlurClass::ShortSteepCrossStaff;
        }
        return PmSlurClass::LocalCrossStaff;
    }
    // Genuine multi-measure phrase slurs flatten; a slur spanning a single
    // measure (~16-19sp in common piano textures, and wider still in loosely
    // justified systems) reads as a local gesture and needs a real arch, so
    // the phrase treatment requires crossing a barline as well as real width.
    if (spanSp >= 20.0
        && (!startCR || !endCR || startCR->measure() != endCR->measure())) {
        return PmSlurClass::LongPhrase;
    }
    return PmSlurClass::Default;
}

double pianomaniaLongPhraseFlatness(double spanSp, double endpointDeltaSp)
{
    const double span = std::max(0.0, spanSp);
    const double tiltRatio = span > 0.5 ? std::abs(endpointDeltaSp) / span : 1.0;
    return std::clamp(1.0 - tiltRatio / 0.30, 0.0, 1.0);
}

// Flattest default arc that still reads as an engraved slur. Collision avoidance only ever
// raises the curve, so this is a starting point, not a cap. Heights are measured perpendicular
// to the endpoint chord; steeply tilted slurs get extra height so the arc stays visible next
// to the strong diagonal of the chord line.
double pianomaniaBaseShoulderHeight(PmSlurClass pmClass, double spanSp, double endpointDeltaSp, double spatium)
{
    const double span = std::max(0.0, spanSp);
    double heightSp = 0.0;
    switch (pmClass) {
    case PmSlurClass::LongPhrase:
        // Long phrase slurs stay restrained: gentle growth well below the default sqrt profile
        heightSp = std::clamp(1.05 + 0.02 * (span - 16.0), 1.05, 1.45);
        break;
    case PmSlurClass::BrokenSegment:
        heightSp = std::clamp(0.55 + 0.055 * span, 0.65, 1.35);
        break;
    case PmSlurClass::ShortSteepCrossStaff:
        heightSp = std::clamp(0.60 + 0.06 * span, 0.70, 1.00);
        break;
    case PmSlurClass::LocalCrossStaff:
        heightSp = std::clamp(0.60 + 0.06 * span, 0.70, 1.30);
        break;
    case PmSlurClass::Default:
        heightSp = std::clamp(0.50 + 0.075 * span, 0.55, 1.30);
        break;
    }

    // Tilt compensation: keep the arc visible against a steep chord line
    const double tilt = span > 0.5 ? std::min(endpointDeltaSp / span, 1.2) : 0.0;
    heightSp *= 1.0 + 0.45 * tilt;

    if (pmClass == PmSlurClass::LongPhrase) {
        const double flatness = pianomaniaLongPhraseFlatness(span, endpointDeltaSp);
        const double spanFactor = std::clamp((span - 20.0) / 20.0, 0.0, 1.0);
        heightSp += (0.30 + 0.10 * spanFactor) * flatness;
    }

    // Omnidirectional bonus: a default slur whose endpoints sit at nearly the
    // same height has no diagonal energy, so the flat-first base reads as a
    // ruler line rather than an arc. Such slurs get extra arch, fading out as
    // the endpoint tilt grows and for very short spans (grace-note slurs).
    if (pmClass == PmSlurClass::Default) {
        const double tiltRatio = span > 0.5 ? endpointDeltaSp / span : 1.0;
        const double flatness = std::clamp(1.0 - tiltRatio / 0.30, 0.0, 1.0);
        const double spanFactor = std::clamp((span - 2.0) / 4.0, 0.0, 1.0);
        heightSp += 0.25 * flatness * spanFactor;
    }

    return heightSp * spatium;
}

// Cosmetic ceiling applied after collision avoidance, and only when re-checking the reduced
// curve against the obstacle shapes proves it stays clear.
double pianomaniaMaxShoulderHeight(PmSlurClass pmClass, double spanSp, double endpointDeltaSp, double spatium)
{
    const double span = std::max(0.0, spanSp);
    double capSp = 0.0;
    switch (pmClass) {
    case PmSlurClass::LongPhrase:
        capSp = 1.70 + 0.40 * pianomaniaLongPhraseFlatness(span, endpointDeltaSp);
        break;
    case PmSlurClass::BrokenSegment:
        capSp = std::clamp(0.30 * span, 1.20, 2.20);
        break;
    case PmSlurClass::ShortSteepCrossStaff:
        capSp = std::clamp(0.35 * span, 1.10, 1.60);
        break;
    case PmSlurClass::LocalCrossStaff:
    case PmSlurClass::Default:
        capSp = std::clamp(0.30 * span, 1.00, 2.40);
        break;
    }
    return capSp * spatium;
}

// Fingering digits are filtered out of the slur obstacle shapes by design
// (fingerings dodge slurs, not the reverse). Wide one-measure Default arches
// are the exception (Arietta m9/m11 "3-[1/4]-3"): the engraved shape is a
// dome over the full figure with its digits inside, so the solver and the
// cosmetic-flatten clearance check must see the digit rects, or the arc gets
// capped below the digit columns and the digits leak above the curve.
bool addPianomaniaSpannedFingeringShapes(const SlurSegment* slurSeg, const ChordRest* startCR, const ChordRest* endCR,
                                         double spatium, Shape& segShapes)
{
    if (!startCR || !endCR || !startCR->segment() || !endCR->segment()) {
        return false;
    }
    bool added = false;
    const staff_idx_t displayStaff = startCR->vStaffIdx();
    const System* system = slurSeg->system();
    for (const Segment* seg = startCR->segment(); seg && seg->tick() <= endCR->tick(); seg = seg->next1enabled()) {
        if (seg->measure()->system() && seg->measure()->system() != system) {
            break;
        }
        ...
        for (const EngravingItem* e : seg->elist()) {
            if (!e || !e->isChord() || toChord(e)->vStaffIdx() != displayStaff) {
                continue;
            }
            for (const Note* note : toChord(e)->notes()) {
                for (EngravingItem* el : note->el()) {
                    if (!el->isFingering() || !el->visible() || !el->addToSkyline()) {
                        continue;
                    }
                    RectF rect = el->ldata()->bbox().translated(
                        seg->pos() + seg->measure()->pos() + e->pos() + note->pos() + el->pos());
                    rect.adjust(0.0, -0.25 * spatium, 0.0, 0.25 * spatium);
                    segShapes.add(ShapeElement(rect, el));
                    added = true;
                }
            }
        }
...
// Shorter default slurs (a measure-long gesture) still need room for the digits of
// their INTERIOR chords: the engraved pocket sits between the notation (stem/beam
// tip) and the slur, and when the slur hugs the beam the pocket is shorter than a
// digit and no tuck is feasible. Instead of the digits' vanilla rects (which may
// already have escaped above the slur and would over-lift it), add synthesized
// digit-sized rects anchored directly on the notation top/bottom — exactly the
// space the tuck needs. Endpoint chords are excluded: takeoff digits dodge
// sideways by design and must not lift the takeoff.
void addPianomaniaPocketFingeringShapes(const SlurSegment* slurSeg, const ChordRest* startCR, const ChordRest* endCR,
                                        bool up, double spatium, Shape& segShapes)
{
    if (!startCR || !endCR || !startCR->segment() || !endCR->segment()) {
        return;
    }
    ...
    for (const Segment* seg = startCR->segment(); seg && seg->tick() < endCR->tick(); seg = seg->next1enabled()) {
        ...
        if (!seg->isChordRestType() || seg->tick() <= startCR->tick()) {
            continue;
        }
        for (const EngravingItem* e : seg->elist()) {
            if (!e || !e->isChord() || toChord(e)->vStaffIdx() != displayStaff) {
            ...
            const PointF chordOrigin = seg->pos() + seg->measure()->pos() + e->pos();
            const RectF chordRect = toChord(e)->ldata()->bbox().translated(chordOrigin);
            for (const Note* note : toChord(e)->notes()) {
                for (EngravingItem* el : note->el()) {
                    if (!el->isFingering() || !el->visible() || !el->addToSkyline()) {
                        continue;
                    }
                    const RectF digitRect = el->ldata()->bbox().translated(
                        chordOrigin + note->pos() + el->pos());
                    // Only digits on the slur's side of the chord need the pocket.
                    const bool digitAbove = digitRect.center().y() < chordRect.center().y();
                    if (digitAbove != up) {
                        continue;
                    }
                    const double height = digitRect.height() + 0.5 * spatium;
                    RectF pocket = digitRect;
                    if (up) {
                        pocket.setTop(chordRect.top() - height);
                        pocket.setBottom(chordRect.top());
                    } else {
                        pocket.setTop(chordRect.bottom());
                        pocket.setBottom(chordRect.bottom() + height);
                    }
                    segShapes.add(ShapeElement(pocket, el));
                }
            }
        }
    }
...
// Outermost obstacle edge (top for up slurs, bottom for down slurs) within an x window.
// Shapes already carry the per-item structural clearance added by addMinClearanceToShapes.
bool pianomaniaObstacleEnvelope(const Shape& segShapes, bool up, double x0, double x1, double& outerY)
{
    bool found = false;
    for (const ShapeElement& el : segShapes.elements()) {
        if (el.right() < x0 || el.left() > x1) {
            continue;
        }
        const double edge = up ? el.top() : el.bottom();
        if (!found || (up ? edge < outerY : edge > outerY)) {
            outerY = edge;
            found = true;
        }
    }
...
// Outer edge (top for up slurs, bottom for down slurs) of the chord's outermost note,
// in the slur segment's grip coordinate frame. Returns false when the anchor is not a chord.
bool pianomaniaAnchorNoteOuterY(const SlurSegment* slurSeg, const ChordRest* cr, bool up, double& outerY)
{
    if (!cr || !cr->isChord()) {
        return false;
    }
    const Chord* chord = toChord(cr);
    const Note* note = up ? chord->upNote() : chord->downNote();
    if (!note) {
        return false;
    }
    const RectF bbox = note->ldata()->bbox();
    outerY = note->pagePos().y() + (up ? bbox.top() : bbox.bottom()) - slurSeg->pagePos().y();
    return true;
}

// Endpoint feasibility for single-system cross-staff slurs, in two steps:
//
// 1) Re-attach: slurPos can leave a cross-staff anchor dangling several spaces toward the
//    other staff, away from its own notehead. Such a takeoff reads as a stray line, so the
//    anchor is pulled back to just outside the note edge.
//
// 2) Hand-off lift: when the slur ends (or starts) on a note displayed on the other staff,
//    deep on the slur's inner side, no smooth arc can dive past the material just before it
//    to reach the notehead. The endpoint is lifted toward the local obstacle envelope so the
//    slur arcs over its own material and reaches toward the hand-off note, which is how
//    engravers treat these gestures.
void anchorPianomaniaCrossStaffEndpoints(const SlurSegment* slurSeg, const ChordRest* startCR, const ChordRest* endCR,
                                         const Shape& segShapes, bool up, double spatium, PointF& pp1, PointF& pp2)
{
    const double outSign = up ? -1.0 : 1.0;

    const double reattachSlack = 1.2 * spatium;
    auto reattach = [&](PointF& endpoint, const ChordRest* cr) {
        double noteEdge = 0.0;
        if (!pianomaniaAnchorNoteOuterY(slurSeg, cr, up, noteEdge)) {
            return;
        }
        if (up ? endpoint.y() > noteEdge + reattachSlack : endpoint.y() < noteEdge - reattachSlack) {
            endpoint.ry() = noteEdge + outSign * 0.25 * spatium;
        }
    };
    reattach(pp1, startCR);
    reattach(pp2, endCR);

    const staff_idx_t startStaff = startCR->vStaffIdx();
    const staff_idx_t endStaff = endCR->vStaffIdx();
    if (startStaff == endStaff) {
        return;
    }

    const double window = 2.0 * spatium;
    const double maxDive = 1.6 * spatium;
    const double maxLift = 5.0 * spatium;

    // A start anchor on the inner staff (rising hand-off) normally takes off
    // from its stem-side anchor over the following material, which is
    // standard engraving and needs no correction. A stemless start chord has
    // no such anchor: slurPos leaves the takeoff at the notehead, deep on the
    // inner side, and the arc cuts through the cross-staff beams right after
    // it (the Arietta m23 closing gesture). Per the keyboard addendum the
    // slur then runs stem-to-notehead: lift the takeoff over the material it
    // must include, like the end-side hand-off lift below.
    const bool startIsInner = up ? startStaff > endStaff : endStaff > startStaff;
    const bool startHasStem = startCR->isChord() && toChord(startCR)->stem();
    if (startIsInner && !startHasStem) {
        // The material the takeoff must clear is the following beamed group,
        // which can sit a full beat away (and the same-tick voice exclusions
        // thin the nearby shapes), so the window is the first half of the
        // span rather than the end-side lift's adjacent 2sp; maxDive/maxLift
        // keep the resulting anchor tied to the takeoff regardless.
        // Unlike the end-side lift, which dives toward its arrival notehead,
        // the takeoff must clear the whole following group, so it rises to
        // the envelope itself (like the broken-slur open-end lift) and its
        // travel bound is looser (the full inter-staff hand-off).
        const double takeoffMaxLift = 8.0 * spatium;
        double envelopeY = 0.0;
        if (pianomaniaObstacleEnvelope(segShapes, up, pp1.x() + 0.3 * spatium, 0.5 * (pp1.x() + pp2.x()), envelopeY)) {
            if (up && pp1.y() > envelopeY + maxDive) {
                pp1.ry() = std::max(envelopeY, pp1.y() - takeoffMaxLift);
            } else if (!up && pp1.y() < envelopeY - maxDive) {
                pp1.ry() = std::min(envelopeY, pp1.y() + takeoffMaxLift);
            }
        }
    }

    // Only the endpoint the slur descends into needs the lift below.
    const bool endIsInner = up ? endStaff > startStaff : endStaff < startStaff;
    if (!endIsInner) {
        return;
    }

    double envelopeY = 0.0;
    if (!pianomaniaObstacleEnvelope(segShapes, up, pp2.x() - window, pp2.x() - 0.3 * spatium, envelopeY)) {
        return;
    }
    if (up && pp2.y() > envelopeY + maxDive) {
        pp2.ry() = std::max(envelopeY + maxDive, pp2.y() - maxLift);
    } else if (!up && pp2.y() < envelopeY - maxDive) {
        pp2.ry() = std::min(envelopeY - maxDive, pp2.y() + maxLift);
    }
}

// Endpoint feasibility for the open ends of broken cross-staff slur segments.
// The open anchor at a system boundary is placed from the last/first ChordRest
// of the slur's own track, but a cross-staff slur's remaining material often
// lives on the other staff, several spaces toward the arc side. A begin
// segment anchored level with its start note then has to climb a wall of
// cross-staff content the solver cannot fully clear, and the arc cuts through
// it. Lift the open endpoint to the local obstacle envelope so the segment
// takes off over its own material, like the single-system hand-off lift.
void liftPianomaniaBrokenCrossStaffOpenEnds(const SlurSegment* slurSeg, const Shape& segShapes, bool up, double spatium,
                                            PointF& pp1, PointF& pp2)
{
    const double window = 2.0 * spatium;
    const double maxLift = 6.0 * spatium;

    auto lift = [&](PointF& endpoint, double x0, double x1) {
        double envelopeY = 0.0;
        if (!pianomaniaObstacleEnvelope(segShapes, up, x0, x1, envelopeY)) {
            return;
        }
        if (up && endpoint.y() > envelopeY) {
            endpoint.ry() = std::max(envelopeY, endpoint.y() - maxLift);
        } else if (!up && endpoint.y() < envelopeY) {
            endpoint.ry() = std::min(envelopeY, endpoint.y() + maxLift);
        }
    };

    const bool openStart = !slurSeg->isSingleBeginType();
    const bool openEnd = !slurSeg->isSingleEndType();
    if (openEnd) {
        lift(pp2, pp2.x() - window, pp2.x() - 0.3 * spatium);
    }
    if (openStart) {
        lift(pp1, pp1.x() + 0.3 * spatium, pp1.x() + window);
    }
}

// Broken slurs must read as one continuous phrase mark (source-of-truth: each
// side of the break is clearly open-ended, no closed hooks). Vanilla layout
// can leave a begin segment's open end diving back toward the staff even when
// the phrase rises into the next system (the Arietta m12 grace → m13 G5 page
// turn), which reads as a finished hook. Each open end is corrected to point
// along the phrase — toward the anchor on the other side of the break — but
// only when it currently points the wrong way, so already-good broken slurs
// replay untouched.
void orientPianomaniaBrokenOpenEnds(const SlurSegment* slurSeg, const ChordRest* startCR, const ChordRest* endCR,
                                    double spatium, PointF& pp1, PointF& pp2)
{
    if (!startCR || !endCR || !startCR->isChord() || !endCR->isChord()) {
        return;
    }
    const Note* startNote = toChord(startCR)->upNote();
    const Note* endNote = toChord(endCR)->upNote();
    if (!startNote || !endNote) {
        return;
    }

    // Only stub segments need explicit orientation: a segment a few spaces
    // wide (a grace group squeezed against the system edge, a lone arrival
    // note after the key signature) is too short to carry an arc, so its
    // endpoint direction is the only thing telling the reader the phrase
    // continues. Longer broken segments carry a real curve and keep their
    // vanilla open ends.
    if (std::abs(pp2.x() - pp1.x()) > 6.0 * spatium) {
        return;
    }

    // Phrase slope across the whole slur in staff steps (frame-free: the two
    // anchors live on different systems). Positive = the slur descends.
    const double slopeSteps = endNote->line() - startNote->line();
    if (std::abs(slopeSteps) < 3.0) {
        return;
    }
    const double drift = 0.8 * spatium;

    if (!slurSeg->isSingleEndType()) {
        // Open end: continue toward the (higher/lower) arrival.
        if (slopeSteps < 0.0 && pp2.y() > pp1.y() - 0.2 * spatium) {
            pp2.ry() = pp1.y() - drift;
        } else if (slopeSteps > 0.0 && pp2.y() < pp1.y() + 0.2 * spatium) {
            pp2.ry() = pp1.y() + drift;
        }
    }
    if (!slurSeg->isSingleBeginType()) {
        // Open start: enter from where the phrase came from.
        if (slopeSteps < 0.0 && pp1.y() < pp2.y() + 0.2 * spatium) {
            pp1.ry() = pp2.y() + drift;
        } else if (slopeSteps > 0.0 && pp1.y() > pp2.y() - 0.2 * spatium) {
            pp1.ry() = pp2.y() - drift;
        }
    }
}

// A slur endpoint must not sit on another voice's notehead. The obstacle
// shapes deliberately exclude same-tick items of other voices (the anchor
// chord's simultaneous partners), so neither slurPos nor the solver ever sees
// them — the Arietta m10 A→G bass slur takes off straight through the
// concurrent E♭ half note. Engraved practice starts the slur just past the
// foreign notehead, so the anchor is nudged horizontally (start → right,
// end → left) out of any such notehead it lands on.
void nudgePianomaniaEndpointsOffForeignNoteheads(const SlurSegment* slurSeg, const ChordRest* startCR, const ChordRest* endCR,
                                                 double spatium, PointF& pp1, PointF& pp2)
{
    auto nudge = [&](PointF& endpoint, const ChordRest* cr, bool start) {
        if (!cr || !cr->isChord() || !cr->segment()) {
            return;
        }
        for (const EngravingItem* e : cr->segment()->elist()) {
            if (!e || e == cr || !e->isChord() || e->track() == cr->track()
                || toChord(e)->vStaffIdx() != cr->vStaffIdx()) {
                continue;
            }
            for (const Note* note : toChord(e)->notes()) {
                RectF rect = note->ldata()->bbox().translated(note->pagePos() - slurSeg->pagePos());
                rect.adjust(-0.1 * spatium, -0.1 * spatium, 0.1 * spatium, 0.1 * spatium);
                if (!rect.contains(endpoint)) {
                    continue;
                }
                endpoint.rx() = start ? rect.right() + 0.1 * spatium : rect.left() - 0.1 * spatium;
            }
        }
    };
    nudge(pp1, startCR, true);
    nudge(pp2, endCR, false);
}

// Same sampling test the collision solver uses, for validating cosmetic changes.
bool pianomaniaCurveClearsShapes(const Shape& segShapes, bool up, const PointF& p2, const PointF& p3, const PointF& p4,
                                 const Transform& toSystemCoordinates, double arcClearance)
{
    const CubicBezier clearanceBezier(PointF(0.0, 0.0), p3 + PointF(0.0, arcClearance), p4 + PointF(0.0, arcClearance), p2);
    static constexpr unsigned npoints = 20;
    PointF prev = toSystemCoordinates.map(clearanceBezier.pointAtPercent(0.0));
    for (unsigned i = 1; i < npoints; ++i) {
        const PointF cur = toSystemCoordinates.map(clearanceBezier.pointAtPercent(double(i) / double(npoints)));
...
// Post-solver smoothness pass, in the rotated slur frame (chord on the x axis, outer side
// negative y for up slurs). Only outward moves are unconditional; every inward (flattening)
// move is validated against the obstacle shapes first.
void reconcilePianomaniaSlurShape(PmSlurClass pmClass, const Shape& segShapes, bool up, double spatium,
                                  double endpointDeltaSp, const PointF& p2, PointF& p3, PointF& p4,
                                  const Transform& toSystemCoordinates, double arcClearance)
{
    const double chordLength = p2.x();
    if (chordLength < 0.5 * spatium) {
        return;
    }

    // No S-curves or endpoint hooks: both control points stay on the outer side
    const double minOuter = 0.35 * spatium;
    if (up) {
        p3.ry() = std::min(p3.y(), -minOuter);
        p4.ry() = std::min(p4.y(), -minOuter);
    } else {
        p3.ry() = std::max(p3.y(), minOuter);
        p4.ry() = std::max(p4.y(), minOuter);
    }

    // Keep the handles ordered and between the endpoints
    p3.rx() = std::clamp(p3.x(), 0.10 * chordLength, 0.55 * chordLength);
    p4.rx() = std::clamp(p4.x(), 0.45 * chordLength, 0.90 * chordLength);
    if (p3.x() > p4.x()) {
        const double mid = 0.5 * (p3.x() + p4.x());
        p3.rx() = mid - 0.05 * chordLength;
        p4.rx() = mid + 0.05 * chordLength;
    }

    // Balance: a control point far shallower than its partner reads as a lopsided arc.
    // Raising the shallow one is always an outward (safe) move. Long slurs may stay
    // lopsided: an arc peaking over the tall end and tapering along the musical line is
    // the engraved shape, and symmetrizing it inflates the middle into a dome.
    const double spanSp = chordLength / spatium;
    auto outerHeight = [up](const PointF& p) { return up ? -p.y() : p.y(); };
    const double h3 = outerHeight(p3);
    const double h4 = outerHeight(p4);
    const double hMax = std::max(h3, h4);
    const double balanceRatio = spanSp < 12.0 ? 0.55 : (spanSp < 24.0 ? 0.40 : 0.25);
    const double minBalanced = balanceRatio * hMax;
    if (h3 < minBalanced) {
        p3.ry() = up ? -minBalanced : minBalanced;
    }
    if (h4 < minBalanced) {
        p4.ry() = up ? -minBalanced : minBalanced;
    }

    // Cosmetic height cap, only applied when the flattened curve still clears everything.
    // Tries a full flatten first, then flattening only the taller handle, so a slur that
    // needs height at one end can still taper at the other.
    const double cap = pianomaniaMaxShoulderHeight(pmClass, spanSp, endpointDeltaSp, spatium);
    const double currentHeight = std::max(outerHeight(p3), outerHeight(p4));
    if (currentHeight <= cap) {
        return;
    }
    auto tryFlatten = [&](const PointF& flat3, const PointF& flat4) {
        if (pianomaniaCurveClearsShapes(segShapes, up, p2, flat3, flat4, toSystemCoordinates, arcClearance)) {
            p3 = flat3;
            p4 = flat4;
            return true;
        }
    ...
    const double fullScale = cap / currentHeight;
    if (tryFlatten(PointF(p3.x(), p3.y() * fullScale), PointF(p4.x(), p4.y() * fullScale))) {
        return;
    }
    const bool p3Taller = outerHeight(p3) >= outerHeight(p4);
    const double tallerReducedY = (up ? -1.0 : 1.0) * std::max(cap, 1.15 * std::min(outerHeight(p3), outerHeight(p4)));
    if (p3Taller ? tryFlatten(PointF(p3.x(), tallerReducedY), p4)
        : tryFlatten(p3, PointF(p4.x(), tallerReducedY))) {
        return;
    }
    const double halfScale = 0.5 * (1.0 + fullScale);
...
void SlurTieLayout::avoidCollisions(SlurSegment* slurSeg, const Shape& segShapes, PointF& pp1, PointF& p2, PointF& p3, PointF& p4,
                                    Transform& toSystemCoordinates, double& slurAngle)
{
    ...
    if (!startCR || !endCR || segShapes.empty()) {
        return;
    }
...
Shape SlurTieLayout::getSegmentShapes(SlurSegment* slurSeg, ChordRest* startCR, ChordRest* endCR, bool localSingleSystemCrossStaff)
{
    Shape segShapes;
    ...
    if (slurSeg->isSingleBeginType()) {
        ...
        startSeg = slurSeg->system()->firstMeasure()->first(); // include clef/key area at system-start continuations
    }
...
Shape SlurTieLayout::getSegmentShape(SlurSegment* slurSeg, Segment* seg, ChordRest* startCR, ChordRest* endCR,
                                     bool localSingleSystemCrossStaff)
{
    Slur* slur = slurSeg->slur();
    ...
    if (slur->isCrossStaff() && (localSingleSystemCrossStaff || seg != startCR->segment())) {
        ...
        if (localSingleSystemCrossStaff) {
            secondStaffShape.remove_if([](ShapeElement& shapeEl) {
                return !keepPianomaniaLocalCrossStaffCollisionShape(shapeEl);
            });
        }
    ...
    for (track_idx_t track = staff2track(startStaffIdx); track < staff2track(endStaffIdx, VOICES); ++track) {
        ...
        if (e->isChord()) {
            ...
            // Articulations that lay away from the note (accents, marcato, …)
            // are not part of the chord or staff shape, so add them here: the
            // arc must clear accents on the notes under the slur. Anchor-chord
            // articulations are excluded again below, which preserves the
            // tucked endpoint behavior handled by layoutArticulations3.
            const System* system = slurSeg->system();
            double staffDiff = 0.0;
            if (system && chord->vStaffIdx() != startStaffIdx
                && chord->vStaffIdx() < system->staves().size() && startStaffIdx < system->staves().size()) {
                staffDiff = system->staves().at(chord->vStaffIdx())->y() - system->staves().at(startStaffIdx)->y();
            }
            const PointF chordOffset = seg->pos() + seg->measure()->pos() + chord->pos() + PointF(0.0, staffDiff);
            for (Articulation* a : chord->articulations()) {
                if (!a->visible() || !a->addToSkyline() || a->isLaissezVib() || a->layoutCloseToNote()) {
                    continue;
                }
                segShape.add(a->shape().translated(a->pos() + chordOffset));
            }
...
void SlurTieLayout::addMinClearanceToShapes(Shape& segShapes, double spatium, bool slurUp, const ChordRest* startCR, const ChordRest* endCR)
{
    ...
    const double clefClearance = 0.62 * spatium;
    const double beamStemClearance = 0.28 * spatium;
    ...
    for (ShapeElement& shapeEl : segShapes.elements()) {
        ...
        case ElementType::NOTE:
            ...
            break;
        case ElementType::CLEF:
            clearance = clefClearance;
            break;
        case ElementType::BEAM:
        case ElementType::HOOK:
        case ElementType::STEM:
            clearance = beamStemClearance;
            break;
        case ElementType::ARTICULATION:
...
void SlurTieLayout::computeBezier(SlurSegment* slurSeg, PointF shoulderOffset)
{
     ...
     * ***********************************************/
    const bool manualSlurEdit = hasPianomaniaManualSlurEdit(slurSeg);
    forceNormalizeManualSlurIfRequested(slurSeg, manualSlurEdit);
    const bool preserveManualSlurShape = manualSlurEdit && !MScore::pianomaniaForceNormalizeSlursFingerings;
    ...
    const bool slurUp = slurSeg->slur()->up();
    ChordRest* startCR = slurSeg->slur()->startCR();
    ChordRest* endCR = slurSeg->slur()->endCR();

    // Obstacle shapes under this segment (system coordinates). Shared by endpoint
    // feasibility, collision avoidance and the post-solver reconciliation.
    Shape segShapes;
    if (startCR && endCR) {
        segShapes = getSegmentShapes(slurSeg, startCR, endCR, isPianomaniaLocalSingleSystemCrossStaff(slurSeg, startCR, endCR));
        addMinClearanceToShapes(segShapes, _spatium, slurUp, startCR, endCR);
    }

    const double chordSpanSp = std::hypot(pp2.x() - pp1.x(), pp2.y() - pp1.y()) / _spatium;
    const double endpointDeltaSp = std::abs(pp2.y() - pp1.y()) / _spatium;
    const PmSlurClass pmClass = startCR && endCR
                                ? classifyPianomaniaSlur(slurSeg, startCR, endCR, chordSpanSp, endpointDeltaSp)
                                : PmSlurClass::Default;
    const bool pmShapingActive = MScore::pianomaniaPrettifySlursFingerings
                                 && !preserveManualSlurShape
                                 && slurSeg->autoplace()
                                 && startCR
                                 && endCR
                                 && !segShapes.empty();

    const bool pmDigitsUnderWideArch = pmShapingActive && pmClass == PmSlurClass::Default && chordSpanSp >= 20.0
                                       && addPianomaniaSpannedFingeringShapes(slurSeg, startCR, endCR, _spatium, segShapes);
    if (pmShapingActive && pmClass == PmSlurClass::Default && chordSpanSp < 20.0) {
        addPianomaniaPocketFingeringShapes(slurSeg, startCR, endCR, slurUp, _spatium, segShapes);
    }

    if (pmShapingActive
        && (pmClass == PmSlurClass::ShortSteepCrossStaff || pmClass == PmSlurClass::LocalCrossStaff)) {
        anchorPianomaniaCrossStaffEndpoints(slurSeg, startCR, endCR, segShapes, slurUp, _spatium, pp1, pp2);
    }
    // Broken-segment orientation needs no obstacle shapes (they are often
    // empty at a system break), so it is not gated on pmShapingActive.
    if (MScore::pianomaniaPrettifySlursFingerings && !preserveManualSlurShape && slurSeg->autoplace()
        && pmClass == PmSlurClass::BrokenSegment) {
        orientPianomaniaBrokenOpenEnds(slurSeg, startCR, endCR, _spatium, pp1, pp2);
    }
    if (pmShapingActive && pmClass == PmSlurClass::BrokenSegment && slurSeg->slur()->isCrossStaff()) {
        liftPianomaniaBrokenCrossStaffOpenEnds(slurSeg, segShapes, slurUp, _spatium, pp1, pp2);
    }
    if (pmShapingActive) {
        nudgePianomaniaEndpointsOffForeignNoteheads(slurSeg, startCR, endCR, _spatium, pp1, pp2);
    }
    ...
    if (pmShapingActive) {
        // Flat-first default: the collision solver below only ever raises the curve,
        // so the default is the flattest arc that still reads as an engraved slur
        const double tiltSp = std::abs(pp2.y() - pp1.y()) / _spatium;
        shoulderH = pianomaniaBaseShoulderHeight(pmClass, d, tiltSp, _spatium);
        if (isOverBeams(slurSeg->slur())) {
            shoulderH = std::max(shoulderH * shoulderReduction, 0.5 * _spatium);
        }
        // A wide one-measure arch with digits inside spans a full local figure
        // (Arietta m9/m11 "3-[1/4]-3"): the engraved shape is the natural dome
        // over the digit columns, not the short-gesture ceiling. Digit-free
        // wide arches keep the flat-first default.
        if (pmDigitsUnderWideArch) {
            shoulderH = std::max(shoulderH, std::min(0.50 + 0.075 * d, 2.40) * _spatium);
        }
    }
    shoulderH -= shoulderOffset.y();
    if (!slurSeg->slur()->up()) {
    ...
    // Edited segments (any persisted grip offset, including offsets written by
    // the persistent Pianomania Prettify) are authoritative: re-running the
    // solver on top of them would compound corrections on every layout, so the
    // curve must replay as default-shape + offset verbatim.
    if (slurSeg->autoplace() && !slurSeg->isEdited()) {
        avoidCollisions(slurSeg, segShapes, pp1, p2, p3, p4, toSystemCoordinates, slurAngle);
    }
    ...
    if (pmShapingActive) {
        const double finalSpanSp = std::abs(p2.x()) / _spatium;
        const double finalEndpointDeltaSp = std::abs(pp2.y() - pp1.y()) / _spatium;
