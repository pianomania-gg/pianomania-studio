...
void SlurTieLayout::computeBezier(SlurSegment* slurSeg, PointF shoulderOffset)
{
    ...
    if (pmShapingActive) {
        ...
        static constexpr double pmShoulderReduction = 0.75;
        if (isOverBeams(slurSeg->slur())) {
            shoulderH = std::max(shoulderH * pmShoulderReduction, 0.5 * _spatium);
        }
    ...
    if (slurSeg->autoplace() && !slurSeg->isEdited() && !slurSeg->isTappingHalfSlurSegment()) {
