...
void TDraw::drawItem(const EngravingItem* item, Painter* painter, const PaintOptions& opt)
{
    ...
    case ElementType::RUBATO_ZONE_SEGMENT:  draw(item_cast<const RubatoZoneSegment*>(item), painter, opt);
        break;

    case ElementType::PYRO_SPAN_SEGMENT:    draw(item_cast<const PyroSpanSegment*>(item), painter, opt);
        break;

    case ElementType::LASER_SPAN_SEGMENT:   draw(item_cast<const LaserSpanSegment*>(item), painter, opt);
        break;
...
void TDraw::draw(const Note* item, Painter* painter, const PaintOptions& opt)
{
    ...
    // NOT tablature
        ...
        int handValue = item->getProperty(Pid::PIANOMANIA_HAND).toInt();
        if (MScore::showPianomaniaHands
            && handValue != static_cast<int>(Note::PianomaniaHand::Undefined)) {
            painter->save();
            Color outlineColor = handValue == static_cast<int>(Note::PianomaniaHand::Left)
                                 ? Color(11, 61, 145, 255)
                                 : Color(194, 90, 0, 255);
            painter->setBrush(BrushStyle::NoBrush);
            painter->setPen(Pen(outlineColor, item->spatium() * 0.12));
            painter->drawEllipse(ldata->bbox());
            painter->restore();
        }

        // Draw held note indicator for PianoMania
        if (item->getProperty(Pid::PIANOMANIA_HELD_NOTE).toBool()) {
            painter->save();
            // Center of the notehead in local coordinates
            double centerX = item->noteheadCenterX();
            double centerY = ldata->bbox().center().y();
            double radius = item->spatium() * 0.25;
            Color circleColor(0, 255, 0, 200); // RGBA, bright green with some transparency
            painter->setBrush(Brush(circleColor));
            painter->setPen(Pen(circleColor, item->spatium() * 0.08));
            painter->drawEllipse(PointF(centerX, centerY), radius, radius);
            painter->restore();
        }

        // Draw bass-shake indicator for PianoMania: a violet ring around the
        // notehead center so it stays legible next to the held-note dot
        if (item->getProperty(Pid::PIANOMANIA_SHAKE_NOTE).toBool()) {
            painter->save();
            double centerX = item->noteheadCenterX();
            double centerY = ldata->bbox().center().y();
            double radius = item->spatium() * 0.42;
            Color ringColor(216, 38, 255, 220); // RGBA, vivid violet
            painter->setBrush(BrushStyle::NoBrush);
            painter->setPen(Pen(ringColor, item->spatium() * 0.1));
            painter->drawEllipse(PointF(centerX, centerY), radius, radius);
            painter->restore();
        }
    }
...
void TDraw::draw(const PyroSpanSegment* item, Painter* painter, const PaintOptions& opt)
{
    TRACE_DRAW_ITEM;
    drawTextLineBaseSegment(item, painter, opt);
}

void TDraw::draw(const LaserSpanSegment* item, Painter* painter, const PaintOptions& opt)
{
    TRACE_DRAW_ITEM;
    drawTextLineBaseSegment(item, painter, opt);
}
