...
static const Color RUBATO_ZONE_COLOR(155, 77, 255); // purple

static const ElementStyle rubatoZoneStyle {
...
RubatoZoneSegment::RubatoZoneSegment(RubatoZone* sp, System* parent)
    : TextLineBaseSegment(ElementType::RUBATO_ZONE_SEGMENT, sp, parent, ElementFlag::MOVABLE | ElementFlag::ON_STAFF)
{
}
...
RubatoZone::RubatoZone(EngravingItem* parent)
    : ChordTextLineBase(ElementType::RUBATO_ZONE, parent)
{
    initElementStyle(&rubatoZoneStyle);
...
static const ElementStyle rubatoZoneSegmentStyle {
...
LineSegment* RubatoZone::createLineSegment(System* parent)
{
    RubatoZoneSegment* rz = new RubatoZoneSegment(this, parent);
    rz->setTrack(track());
    rz->initElementStyle(&rubatoZoneSegmentStyle);
    return rz;
}
...
PropertyValue RubatoZone::propertyDefault(Pid propertyId) const
{
    switch (propertyId) {
    case Pid::COLOR:
        return PropertyValue::fromValue(RUBATO_ZONE_COLOR);
    ...
    // "[" and "]" bracketing the zone from above.
