...
{
    if (m_trillType == TrillType::TURN_BETWEEN) {
        ...
        System* startSys = startSeg->measure()->system();
        System* endSys = endSeg->measure()->system();
        if (!startSys || !endSys || startSys != endSys) {
            return trillLinePos(this, grip, system);
        }

        double startX = startSeg->x() + startSeg->measure()->x();
        double endX = endSeg->x() + endSeg->measure()->x();
        double midX = (startX + endX) * 0.5;
        *system = startSys;
        return PointF(midX, 0.0);
    }
