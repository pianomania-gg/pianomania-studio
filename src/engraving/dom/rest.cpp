...
void Rest::setTrack(track_idx_t val)
{
    ...
    if (m_pianomaniaHand == static_cast<int>(Note::PianomaniaHand::Undefined) && staff() && staff()->part()) {
        const std::vector<Staff*>& partStaves = staff()->part()->staves();
        if (!partStaves.empty()) {
            if (partStaves.back() == staff()) {
                m_pianomaniaHand = static_cast<int>(Note::PianomaniaHand::Left);
            } else if (partStaves.front() == staff()) {
                m_pianomaniaHand = static_cast<int>(Note::PianomaniaHand::Right);
            } else {
                m_pianomaniaHand = static_cast<int>(Note::PianomaniaHand::Right);
            }
        }
