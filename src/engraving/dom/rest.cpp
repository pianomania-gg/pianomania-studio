...
void Rest::setTrack(track_idx_t val)
{
    ...
    if (m_pianomaniaHand == static_cast<int>(Note::PianomaniaHand::Undefined)) {
        assignPianomaniaHandFromStaff();
    }
}

void Rest::assignPianomaniaHandFromStaff()
{
    if (!staff() || !staff()->part()) {
        return;
    }
    ...
    const Note::PianomaniaHand hand = partStaves.back() == staff()
        ? Note::PianomaniaHand::Left
        : Note::PianomaniaHand::Right;
    m_pianomaniaHand = static_cast<int>(hand);
}
...
PropertyValue Rest::propertyDefault(Pid propertyId) const
{
    ...
    case Pid::PIANOMANIA_HAND:
        return static_cast<int>(Note::PianomaniaHand::Undefined);
