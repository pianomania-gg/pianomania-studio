...
static const Color LASER_SPAN_COLOR(0, 229, 255); // cyan

static const ElementStyle laserSpanStyle {
...
LaserSpanSegment::LaserSpanSegment(LaserSpan* sp, System* parent)
    : TextLineBaseSegment(ElementType::LASER_SPAN_SEGMENT, sp, parent, ElementFlag::MOVABLE | ElementFlag::ON_STAFF)
{
}
...
LaserSpan::LaserSpan(EngravingItem* parent)
    : ChordTextLineBase(ElementType::LASER_SPAN, parent)
{
    initElementStyle(&laserSpanStyle);
...
static const ElementStyle laserSpanSegmentStyle {
...
LineSegment* LaserSpan::createLineSegment(System* parent)
{
    LaserSpanSegment* ls = new LaserSpanSegment(this, parent);
    ls->setTrack(track());
    ls->initElementStyle(&laserSpanSegmentStyle);
    return ls;
}
...
PropertyValue LaserSpan::propertyDefault(Pid propertyId) const
{
    switch (propertyId) {
    case Pid::COLOR:
        return PropertyValue::fromValue(LASER_SPAN_COLOR);
