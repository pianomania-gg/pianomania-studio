...
void Note::setupAfterRead(const Fraction& ctxTick, bool pasteMode)
{
    ...
    if (m_pianomaniaHand == PianomaniaHand::Undefined && staff() && staff()->part()) {
        const std::vector<Staff*>& partStaves = staff()->part()->staves();
        if (!partStaves.empty()) {
            if (partStaves.back() == staff()) {
                m_pianomaniaHand = PianomaniaHand::Left;
            } else if (partStaves.front() == staff()) {
                m_pianomaniaHand = PianomaniaHand::Right;
            } else {
                m_pianomaniaHand = PianomaniaHand::Right;
            }
        }
