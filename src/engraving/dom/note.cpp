    ...
    : EngravingItem(n, link)
{
    ...
    m_pianomaniaHeldNote = n.m_pianomaniaHeldNote;
    m_pianomaniaHeldNotePulse = n.m_pianomaniaHeldNotePulse;
    m_pianomaniaHeldNotePulseTriplet = n.m_pianomaniaHeldNotePulseTriplet;
    m_pianomaniaHeldNotePitchCurve = n.m_pianomaniaHeldNotePitchCurve;
    m_pianomaniaHand    = n.m_pianomaniaHand;
    m_pianomaniaShakeNote = n.m_pianomaniaShakeNote;
...
void Note::setupAfterRead(const Fraction& ctxTick, bool pasteMode)
{
    ...
    if (pasteMode || m_pianomaniaHand == PianomaniaHand::Undefined) {
        assignPianomaniaHandFromStaff();
    }
...
void Note::assignPianomaniaHandFromStaff()
{
    if (!staff() || !staff()->part()) {
        return;
    }

    const std::vector<Staff*>& partStaves = staff()->part()->staves();
    if (partStaves.empty()) {
        return;
    }

    m_pianomaniaHand = partStaves.back() == staff() ? PianomaniaHand::Left : PianomaniaHand::Right;
}
...
PropertyValue Note::getProperty(Pid propertyId) const
{
    ...
    case Pid::FIXED_LINE:
        return fixedLine();
    case Pid::PIANOMANIA_HELD_NOTE:
        return m_pianomaniaHeldNote;
    case Pid::PIANOMANIA_HELD_NOTE_PULSE:
        return static_cast<int>(m_pianomaniaHeldNotePulse);
    case Pid::PIANOMANIA_HELD_NOTE_PULSE_TRIPLET:
        return m_pianomaniaHeldNotePulseTriplet;
    case Pid::PIANOMANIA_HELD_NOTE_PITCH_CURVE:
        return m_pianomaniaHeldNotePitchCurve;
    case Pid::PIANOMANIA_HAND:
        return static_cast<int>(m_pianomaniaHand);
    case Pid::PIANOMANIA_SHAKE_NOTE:
        return m_pianomaniaShakeNote;
...
bool Note::setProperty(Pid propertyId, const PropertyValue& v)
{
    ...
    case Pid::FIXED_LINE:
        ...
        break;
    case Pid::PIANOMANIA_HELD_NOTE:
        setPianomaniaHeldNote(v.toBool());
        break;
    case Pid::PIANOMANIA_HELD_NOTE_PULSE:
        setPianomaniaHeldNotePulse(static_cast<PianomaniaHeldNotePulse>(v.toInt()));
        break;
    case Pid::PIANOMANIA_HELD_NOTE_PULSE_TRIPLET:
        setPianomaniaHeldNotePulseTriplet(v.toBool());
        break;
    case Pid::PIANOMANIA_HELD_NOTE_PITCH_CURVE: {
        const PianomaniaHeldNotePitchCurve curve = v.value<PianomaniaHeldNotePitchCurve>();
        if (!isValidPianomaniaHeldNotePitchCurve(curve)) {
            return false;
        }
        setPianomaniaHeldNotePitchCurve(curve);
        break;
    }
    case Pid::PIANOMANIA_HAND:
        setPianomaniaHand(static_cast<PianomaniaHand>(v.toInt()));
        break;
    case Pid::PIANOMANIA_SHAKE_NOTE:
        setPianomaniaShakeNote(v.toBool());
        break;
    case Pid::HAS_PARENTHESES:
...
PropertyValue Note::propertyDefault(Pid propertyId) const
{
    ...
    case Pid::PIANOMANIA_HELD_NOTE:
        return false;
    case Pid::PIANOMANIA_HELD_NOTE_PULSE:
        return static_cast<int>(PianomaniaHeldNotePulse::None);
    case Pid::PIANOMANIA_HELD_NOTE_PULSE_TRIPLET:
        return false;
    case Pid::PIANOMANIA_HELD_NOTE_PITCH_CURVE:
        return PianomaniaHeldNotePitchCurve();
    case Pid::PIANOMANIA_HAND:
        return static_cast<int>(PianomaniaHand::Undefined);
    case Pid::PIANOMANIA_SHAKE_NOTE:
        return false;
    case Pid::TPC2:
...
int Note::pianomaniaHeldNoteDurationTicks() const
{
    const std::vector<Note*> chain = tiedNotes();
...
bool Note::isValidPianomaniaHeldNotePitchCurve(const PianomaniaHeldNotePitchCurve& curve, int durationTicks)
{
    if (curve.empty()) {
        return true;
    }
    if (curve.size() < 2 || curve.size() > 32 || curve.front().scoreTick != 0 || curve.front().pitchCents != 0
        || curve.back().scoreTick <= 0 || (durationTicks > 0 && curve.back().scoreTick != durationTicks)) {
        return false;
    }
    ...
    for (const PianomaniaHeldNotePitchCurvePoint& point : curve) {
        if (point.scoreTick < 0 || point.scoreTick <= previousTime
            || std::abs(point.pitchCents) > PianomaniaHeldNotePitchCurvePoint::MAX_PITCH_CENTS
            || std::abs(point.slopeCentsPerQuarter) > PianomaniaHeldNotePitchCurvePoint::MAX_SLOPE_CENTS_PER_QUARTER) {
            return false;
        }
        previousTime = point.scoreTick;
        hasNonzeroPitch = hasNonzeroPitch || point.pitchCents != 0;
    }
    for (size_t i = 0; i + 1 < curve.size(); ++i) {
        const auto& a = curve[i];
        const auto& b = curve[i + 1];
        const double h = static_cast<double>(b.scoreTick - a.scoreTick) / Constants::DIVISION;
        const double c0 = a.pitchCents;
        const double c1 = h * a.slopeCentsPerQuarter;
        const double c2 = -3.0 * a.pitchCents - 2.0 * c1 + 3.0 * b.pitchCents
                          - h * b.slopeCentsPerQuarter;
        const double c3 = 2.0 * a.pitchCents + c1 - 2.0 * b.pitchCents
                          + h * b.slopeCentsPerQuarter;
        const auto inRange = [&](double t) {
            const double pitch = ((c3 * t + c2) * t + c1) * t + c0;
            return pitch >= -PianomaniaHeldNotePitchCurvePoint::MAX_PITCH_CENTS - 1e-7
                   && pitch <= PianomaniaHeldNotePitchCurvePoint::MAX_PITCH_CENTS + 1e-7;
        };
        const double discriminant = 4.0 * c2 * c2 - 12.0 * c3 * c1;
        if (std::abs(c3) < 1e-12) {
            if (std::abs(c2) >= 1e-12) {
                const double root = -c1 / (2.0 * c2);
                if (root > 0.0 && root < 1.0 && !inRange(root)) return false;
            }
        } else if (discriminant >= 0.0) {
            const double sqrtDiscriminant = std::sqrt(discriminant);
            for (double root : { (-2.0 * c2 + sqrtDiscriminant) / (6.0 * c3),
                                 (-2.0 * c2 - sqrtDiscriminant) / (6.0 * c3) }) {
                if (root > 0.0 && root < 1.0 && !inRange(root)) return false;
            }
        }
    }
    return hasNonzeroPitch;
}
