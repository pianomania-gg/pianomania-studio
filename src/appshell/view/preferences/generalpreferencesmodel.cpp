...
void GeneralPreferencesModel::quitApp()
{
    std::string title = muse::trc("appshell/preferences", "Exit Pianomania?");
    std::string body = muse::trc("appshell/preferences", "Are you sure you want to exit?");

    IInteractive::ButtonData cancelBtn = interactive()->buttonData(IInteractive::Button::Cancel);
    cancelBtn.accent = true;

    int exitBtn = int(IInteractive::Button::Apply);
    IInteractive::Result result = interactive()->warning(title, body,
        { cancelBtn, IInteractive::ButtonData(exitBtn, muse::trc("appshell/preferences", "Exit")) },
