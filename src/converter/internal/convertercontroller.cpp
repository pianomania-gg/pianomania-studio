...
Ret ConverterController::fileConvert(const path_t& in, const path_t& out,
                                     ...
                                     bool pianomaniaAutoLayout,
                                     bool pianomaniaPrettify)
{
                                     ...
                                     bool pianomaniaPrettify)
{
    ...
    if (pianomaniaAutoLayout) {
        ret = ConverterUtils::applyPianomaniaAutoLayout(notationProject->masterNotation()->notation());
        if (!ret) {
            LOGE() << "Failed to apply Pianomania auto-layout, err: " << ret.toString();
            return ret;
        }
    }

    if (pianomaniaPrettify) {
        ret = ConverterUtils::applyPianomaniaPrettify(notationProject->masterNotation()->notation());
        if (!ret) {
            LOGE() << "Failed to apply Pianomania prettify, err: " << ret.toString();
            return ret;
        }
    }
...
Ret ConverterController::exportPianomaniaMidi(const muse::io::path_t& in, const muse::io::path_t& basePath,
                                              ...
                                              bool pianomaniaPrettify)
{
    ...
    if (basePath.empty() || meiPath.empty() || manifestPath.empty()) {
        return make_ret(Err::UnknownError, "Pianomania export requires base, MEI, and manifest paths");
    }
    ...
    mu::engraving::Score* score = notation ? notation->elements()->msScore() : nullptr;
    if (!score) {
        return make_ret(Err::UnknownError, "No score available for Pianomania MIDI export");
    }

    const bool exportRpns = midiImportExportConfiguration()->isMidiExportRpns();
    RetVal<project::pianomania::PianomaniaExportResult> exportResult
        = project::pianomania::exportPianomaniaBundle(score, in, basePath, meiPath, exportRpns);
    if (!exportResult.ret) {
        return make_ret(Err::OutFileFailedWrite, exportResult.ret.text());
    }

    if (!project::pianomania::writePianomaniaManifest(manifestPath, exportResult.val)) {
        LOGE() << "failed write Pianomania manifest, path: " << manifestPath;
        return make_ret(Err::OutFileFailedWrite, manifestPath.toStdString());
    }

    if (!pdfPath.empty()) {
        auto pdfWriter = writers()->writer(PDF_SUFFIX);
        if (!pdfWriter) {
            return make_ret(Err::ConvertTypeUnknown);
        }
        ret = convertFullNotation(pdfWriter, notation, pdfPath);
        if (!ret) {
            LOGE() << "failed write Pianomania PDF, err: " << ret.toString() << ", path: " << pdfPath;
            return ret;
        }
    }
