...
protected:
    ...
    int _len { 0 };
    int _metaType { 0 };
    const engraving::Note* _pianomaniaSourceNote { nullptr };
    std::vector<MidiNoteOrigin> _pianomaniaNoteOrigins;
...
public:
    ...
    const engraving::Note* pianomaniaSourceNote() const { return _pianomaniaSourceNote; }
    void setPianomaniaSourceNote(const engraving::Note* note) { _pianomaniaSourceNote = note; }
    const std::vector<MidiNoteOrigin>& pianomaniaNoteOrigins() const { return _pianomaniaNoteOrigins; }
    void setPianomaniaNoteOrigins(const std::vector<MidiNoteOrigin>& origins) { _pianomaniaNoteOrigins = origins; }
};
}
