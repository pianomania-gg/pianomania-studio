...
namespace {
bool shouldExportNote(const Note *note) { return note && note->visible(); }
  ...
  for (size_t j = 0; j < staff->bracketLevels() + 1; j++) {
    if (staff->bracketType(j) != BracketType::NO_BRACKET) {
      libmei::StaffGrp meiStaffGrp =
          Convert::bracketToMEI(staff->bracketType(j), staff->barLineSpan());
 ...
 */
void MeiExporter::collectNoteEvents(const Measure *measure) {
      ...
      for (Segment *segment = measure->first(); segment;
        ...
        if (engravingItem->isChord()) {
          ...
          for (const Chord *graceChord : chord->graceNotesBefore())
            pushChordNotes(graceChord);

          pushChordNotes(chord);
 ...
 */
void MeiExporter::assignIndicesForMeasure(const Measure *measure) {
  collectNoteEvents(measure);
...
bool MeiExporter::writeMeasure(const Measure *measure, int &measureN,
  ...
  assignIndicesForMeasure(measure);
  ...
  for (auto controlEvent : m_startingControlEventList) {
    if (controlEvent.first->isArpeggio()) {
      ...
      success =
          success &&
          this->writeDynam(dynamic_cast<const Dynamic *>(controlEvent.first),
                           controlEvent.second);
...
bool MeiExporter::writeBeamAndTuplet(const ChordRest *chordRest,
  ...
  if (closingTuplet && closingBeam &&
      beam->elements().size() < tuplet->elements().size()) {
    closingBeam = false;
...
bool MeiExporter::writeGraceChord(const Chord *graceChord,
  ...
  if (isChord) {
    ...
    this->fillControlEventMap(xmlId, graceChord);
  }
...
bool MeiExporter::writeHairpin(const Hairpin *hairpin,
  ...
  // Get the parent segment.
  const Segment *segment = hairpin->startSegment();
  if (segment) {
    double yPos = getHairpinYOffset(hairpin);

    // Add yOffset as a custom attribute using formatDecimalStr
    hairpinNode.append_attribute("yOffset") = formatDecimalStr(yPos, 1);

    // Export whether the hairpin should be centered between staves.
    AutoOnOff centerValue = hairpin->centerBetweenStaves();
    if (centerValue == AutoOnOff::ON) {
      hairpinNode.append_attribute("centerBetweenStaves") = "true";
    } else if (centerValue == AutoOnOff::OFF) {
      hairpinNode.append_attribute("centerBetweenStaves") = "false";
    }
  }
  ...
  if (!hairpin->segmentsEmpty()) {
    ...
    const HairpinSegment *firstSegment = hairpin->frontSegment()
                                             ? toHairpinSegment(hairpin->frontSegment())
                                             : nullptr;
    ...
    if (firstSegment && lastSegment) {
      ...
      for (const SpannerSegment *seg : hairpin->spannerSegments()) {
        ...
        int pageIndex = segmentPage ? static_cast<int>(segmentPage->no()) : 0;

        const PointF *linePoints = hairpinSeg->points();
        if (linePoints && hairpinSeg->npoints() >= 4) {
...
MeiExporter::collectTieContextNotes(const Tie *tie) const {
  ...
  auto isCandidateNote = [&](const Note *candidate) {
    if (!candidate || candidate == startNote || candidate == endNote) {
      return false;
    }
...
bool MeiExporter::writeTie(const Tie *tie, const std::string &startid) {
  ...
  PointF startPos = tie->startNote()->pagePos();
  PointF endPos = tie->endNote()->pagePos();
...
void MeiExporter::fillControlEventMap(const std::string &xmlId,
  ...
  auto spanners = smap.findOverlapping(chordRest->tick().ticks(),
                                       chordRest->tick().ticks());
  for (auto interval : spanners) {
    Spanner *spanner = interval.value;
    if (spanner &&
...
static double calculateDirectiveYOffset(const EngravingItem *dir,
  ...
  const double directiveY = dir->y() + dir->offset().y();

  PlacementV placement = dir->placement();
  double anchorY =
      (placement == PlacementV::BELOW) ? anchor->downPos() : anchor->upPos();

  return -(directiveY - anchorY) / lineDist;
}
...
void MeiExporter::writeSurfaceSizeInches(pugi::xml_node &meiHead) {
  ...
  surface.append_attribute("pm:dpi") = std::to_string(DPI).c_str();
