...
private:
  bool writeHeader();
  bool validateRubatoZones();
  bool validatePianomaniaHeldPitchCurves();
  bool validateDanceShowSpans();
  void prepareSpannerEndpointAnchors();
  bool writeScore();
  bool writeScoreDef();
  ...
  bool writeGliss(const engraving::Glissando *gliss,
                  const std::string &startid);
  ...
  bool writeRubatoZone(const engraving::RubatoZone *rubatoZone,
                       const std::string &startid);
  bool writeDanceShowSpan(const engraving::Spanner *span,
                          const std::string &startid, const char *meiType,
                          char xmlIdPrefix);
  ...
  void fillControlEventMap(const std::string &xmlId,
                           ...
                           bool includeSegmentAnnotations = true);
   ...
   */
  bool collectNoteEvents(const engraving::Measure *measure);
  bool assignIndicesForMeasure(const engraving::Measure *measure);
  ...
  /** Exported boundary anchors for complete-measure rubato zones. */
  std::map<const engraving::RubatoZone *, const engraving::ChordRest *>
      m_rubatoZoneStartAnchors;
  std::map<const engraving::RubatoZone *, const engraving::ChordRest *>
      m_rubatoZoneEndAnchors;

  /** Exported reference anchors for dance-show effect spans (pyro, laser). */
  std::map<const engraving::Spanner *, const engraving::ChordRest *>
      m_danceShowStartAnchors;
  std::map<const engraving::Spanner *, const engraving::ChordRest *>
      m_danceShowEndAnchors;
  std::multimap<const engraving::ChordRest *, engraving::Spanner *>
      m_spannerStartAnchors;
  std::multimap<const engraving::ChordRest *, engraving::Spanner *>
      m_spannerEndAnchors;
